...
class facturas extends ConexionClass {
   private $codresult;
	...
	public function datosFacturaPdf($proyecto, $periodo, $grupo,$zona){

        if ($grupo != '') {
            $where .= " AND C.GRUPO = '$grupo' ";
        }
        if ($zona != '') {
            $where .= " AND I.ID_ZONA = '$zona' ";
        }


        $sql = "SELECT
                                                                         TD.DESCRIPCION_TIPO_DOC TIPODOC,
                                                                                                              TO_CHAR(f.VENCIMIENTO_NCF,'DD/MM/YYYY') VENCE_NCF,i.ID_PROYECTO, F.MSJ_NCF,
  I.CODIGO_INM,  CONCAT(N.ID_NCF,F.NCF_CONSEC) NCF, I.CATASTRO, C.ALIAS, I.DIRECCION, U.DESC_URBANIZACION, I.ID_ZONA,
...
WHERE G.ID_GERENCIA = Z.ID_GERENCIA AND Z.ID_ZONA = I.ID_ZONA) GERENCIA, E.DESC_MED, M.COD_CALIBRE, M.SERIAL, TO_CHAR(F.FEC_VCTO,'DD/MM/YYYY')FEC_VCTO,
                                                                                                              TO_CHAR(FECHA_CORTE,'DD/MM/YYYY')FECCORTE, F.CONSEC_FACTURA,
                                                                                                              ...
                                                                                                              (SELECT COUNT(FA.FACTURA_PAGADA)
                                                                                                               ...
                                                                                                               WHERE FA.INMUEBLE = F.INMUEBLE
                                                                                                                     ...
                                                                                                                     AND FA.PERIODO <> $periodo) FACPEND,
                                                                                                              (SELECT COUNT(FA.FACTURA_PAGADA)
                                                                                                               FROM SGC_TT_FACTURA FA
                                                                                                               WHERE FA.INMUEBLE = F.INMUEBLE
                                                                                                                     AND FA.PERIODO <> $periodo) FACGEN,
                                                                                                              (SELECT SUM(D.VALOR)
                                                                                                               FROM SGC_TT_DETALLE_FACTURA D
                                                                                                               WHERE D.COD_INMUEBLE = F.INMUEBLE
                                                                                                                     AND D.PAGADO = 'N'
                                                                                                                     AND D.CONCEPTO = 10
                                                                                                                     AND PERIODO <> $periodo) MORA,
                                                                                                              (SELECT SUM(FA.TOTAL)
                                                                                                               FROM SGC_TT_FACTURA FA
                                                                                                               WHERE FA.INMUEBLE = F.INMUEBLE
                                                                                                                     AND FA.FACTURA_PAGADA = 'N'
                                                                                                                     AND FA.PERIODO <> $periodo) DEUDA
