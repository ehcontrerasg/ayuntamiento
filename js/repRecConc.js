...
function genRep() {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            divRepPagConcRep.data = datos;
        }
    }
    xmlhttp.open("GET", '../reportes/reporte.recaudoPorConcepto.php?fecPagIni=' + fecIni + '&fecPagFin=' + fecFin +
    "&idConcepto=" + idConcepto + "&descConcepto=" + textConc + "&acueducto=" + acueducto + "&entIni=" + entIni +
    "&entFin=" + entFin + "&punIni=" + punIni + "&punFin=" + punFIn + "&cajaIni=" + cajaIni + "&cajaFin=" + cajaFIn, true);   // async
    xhttp.send();
}
