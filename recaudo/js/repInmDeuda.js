...
function generaRep(){
    var datos=$("#genRepInmDeudaForm").serializeArray();
    $.ajax
    ({
        url : '../reportes/reportes.Inmuebles_Deuda.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.datRepInmDeuda.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {
            $('#selProyInmDeuda').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProyInmDeuda').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
