...
function generaRep(){
    var datos=$("#repPagPorFecDetForm").serializeArray();

    $.ajax
    ({
        url : '../reportes/reporte.repPagoFechaDet.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.repPagoFechaDet.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {
            $('#repPagPorFecDetPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repPagPorFecDetPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
