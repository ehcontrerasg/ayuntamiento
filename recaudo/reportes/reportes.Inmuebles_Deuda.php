...
if (trim($periodo1) != "" && trim($periodo2)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->getStyle('A1:T2')->applyFromArray($estiloTitulos);
...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('T2', 'Categoría');
if (trim($periodo1) != "" && trim($periodo2)) {
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('U2', 'Recaudo periodo ' . $periodo1)
        ->setCellValue('V2', 'Recaudo periodo ' . $periodo2);
}
...
while (oci_fetch($registros)) {
    ...
    if ($facpend >= $facIni && $facpend <= $facFin) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('T' . $fila, $categoria);
        if (trim($periodo1) != "" && trim($periodo2)) {
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('U' . $fila, $rec1)
                ->setCellValue('V' . $fila, $rec2);
        }
        $fila++;
    }
}
