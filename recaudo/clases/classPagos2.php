...
class Pagos extends ConexionClass
{
    ...
    {
        $sql = "SELECT I.CODIGO_INM, I.DIRECCION, U.DESC_URBANIZACION, C.CODIGO_CLI, C.ALIAS, I.ID_ESTADO, A.DESC_ACTIVIDAD, A.ID_USO, I.ID_PROYECTO, P.DESC_PROYECTO, L.NOMBRE_CLI, I.ID_TIPO_CLIENTE,
                   SI.COD_SERVICIO 
...
*/
    ...
    public function seleccionaConcepto2()
    {
        $sql = "SELECT COD_SERVICIO, DESC_SERVICIO
        FROM SGC_TP_SERVICIOS
        WHERE OTRO_RECAUDO = 'S' AND COD_SERVICIO NOT IN (1,20) ORDER BY COD_SERVICIO";
        $resultado = oci_parse($this->_db, $sql);
        $banderas  = oci_execute($resultado);
        if ($banderas == true) {
            oci_close($this->_db);
