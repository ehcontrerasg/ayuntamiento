...
if ($verificarPermisos==true):
    ...
    $largo = 240;
    $c = new Pagos();
    $resultado = $c->obtenerMaxPagoPromotora($cod_inmueble);
    while (oci_fetch($resultado)) {
        $id_pago = oci_result($resultado, 'ID_PAGO');
    }oci_free_statement($resultado);
    $c = new Pagos();
   /* $resultado = $c->obtenerCantidadDatosPagoPromotora($id_pago);
    while (oci_fetch($resultado)) {
    ...
    $c = new Pagos();
    $resultado = $c->obtenerFechaPagoPromotora($id_pago);
    while (oci_fetch($resultado)) {
    ...
    $c = new Pagos();
    $resultado = $c->seleccionaDatosReciboPagoPromotora($id_pago);
    while (oci_fetch($resultado)) {
        $login = oci_result($resultado, 'ID_USUARIO');
        $importe = oci_result($resultado, 'VALOR_IMPORTE');
    }oci_free_statement($resultado);
    ...
    $pdf->SetFont('Arial','',7);
    $pdf->Text(3, 29, 'VALORES PAGADOS EN ESTE RECIBO');
    ...
    $pdf->Image($imagenrec,3,$px+14,4);
    $pdf->Rect(0.5,$px+12,52,9);
    $pdf->SetTextColor(19,30,181);
    $pdf->Text(25, $px + 15,'PAGADO');
    $pdf->SetTextColor(64,65,68);
    $pdf->Text(19, $px + 18,$fecha);
    $pdf->SetFont('Arial','',6);
