...
//require_once('../../destruye_sesion_cierra.php');
    ...
    while (oci_fetch($resultado)) {
        ...
        if($acueducto == 'SD'){
            ...
            $pdf->Text(38,15,"CORPORACI�N DE ACUEDUCTO Y ALCANTARILLADO DE SANTO DOMINGO (CAASD)");
            $pdf->Text(92,22,"GERENCIA COMERCIAL");
            $pdf->Text(58,29,"CONSTANCIA DE RECEPCI�N DE RECLAMOS Y SOLICITUDES");
            $pdf->Image("../../images/logo_caasd2.jpg",14,9,20);
        }

        if($acueducto == 'BC'){
            $pdf->SetFont('Helvetica','B',11);
            $pdf->Text(38,15,"CORPORACI�N DE ACUEDUCTO Y ALCANTARILLADO DE BOCACHICA (CORAABO)");
            $pdf->Text(92,22,"GERENCIA COMERCIAL");
            $pdf->Text(58,29,"CONSTANCIA DE RECEPCI�N DE RECLAMOS Y SOLICITUDES");
            $pdf->Image("../../images/logo_coraabo.jpg",10,15,40);
        }
        ...
        $pdf->Ln(5);
        $pdf->Cell(190,5,"P�g:      ".$pdf->PageNo().' / {nb}',0,0,'R');
        ...
        $pdf->Cell(30,5,"Fecha Recepci�n: ",0,0,'L');
        $pdf->SetFont('Arial','',8);
        ...
        $pdf->Ln(4);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(30,5,"Descripci�n: ",0,0,'L');
        $pdf->Ln(4);
        $pdf->SetFont('Arial','',8);
        $descrip1 = substr($descripcion,0,117);
        $descrip2 = substr($descripcion,117,strlen($descripcion));
        ...
        $pdf->Ln(23);
        $pdf->SetFont('Arial','B',9);
        $pdf->Cell(190,5,"DATOS DE RECEPCI�N",1,0,'C',true);
        ...
        $pdf->Ln(4);
        $pdf->Cell(190,5,"mayor  a quince (15) d�as a  partir de la  fecha de recepci�n.  Si transcurre  dicho plazo y  usted no ha recibido  respuesta, favor llamar al  telefono",0,0,'L');
        $pdf->Ln(4);
        $pdf->Cell(190,5,"(809) 598-1722 Opción 2.",0,0,'L');
        $pdf->Ln(6);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(190,5,"SU SATISFACCI�N ES NUESTRO COMPROMISO",0,1,'C');

        $pdf->Ln(3);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(40,5,"Fecha L�mite de Respuesta: ",0,0,'L');
        $pdf->SetFont('Arial','',8);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(190,5,"Fecha de Resoluci�n:   _________________________________________________________ ",0,0,'L');
