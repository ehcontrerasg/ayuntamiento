...
function generaRep(){
    var datos=$("#genRepHisFacForm").serializeArray();
    $.ajax
    ({
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selProyHisFac').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProyHisFac').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
