...
function generaRep(){
    var datos=$("#genRepSegRecForm").serializeArray();
    $.ajax
    ({
        url : '../reportes/reportes.Seguimiento_recaudo.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selProySegRec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProySegRec').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
