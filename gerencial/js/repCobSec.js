...
function generaRep(){
    var datos=$("#genRepCobSecForm").serializeArray();
    $.ajax
    ({
        url : '../reportes/reportes.CobroSector.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selProyCobSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProyCobSec').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
