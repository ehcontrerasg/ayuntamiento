...
function generaRep(){
    var datos=$("#genRepHisRecForm").serializeArray();
    $.ajax
    ({
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selProyHisRec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProyHisRec').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
