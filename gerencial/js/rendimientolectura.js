...
function generaRep(){
    ...
    $.ajax
    ({
        url : '../reportes/reportes.rendimientoLectura.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSelEmpLec()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selEmpLec',pro:$("#selProyAntMed").val()},
        success : function(json) {
            $('#selEmpAntMed').empty();
            $('#selEmpAntMed').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selEmpAntMed').append(new Option(json[x]["NOM_USUARIO"], json[x]["ID_USUARIO"], false, false));
            }
        },
        error : function(xhr, status) {
