...
function generaRep(){
    var datos=$("#genRepFacRecDetUsoForm").serializeArray();
    $.ajax
    ({
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selProyFacRecDetUso').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selProyFacRecDetUso').append(new Option(json[x]["SIGLA_PROYECTO"], json[x]["ID_PROYECTO"], false, false));
            }
        },
        error : function(xhr, status) {
