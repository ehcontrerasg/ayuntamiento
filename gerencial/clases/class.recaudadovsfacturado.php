<?php
    ...
    class RecaudadoVsFacturado extends ConexionClass {
        ...
        function gatCantidadPagos($proyecto="",$periodo_inicial, $periodo_final){
            ...
            $sql="select  SUM(CANTIDAD)CANTIDAD, SUM(IMPORTE)IMPORTE
                        ...
                        AND TO_CHAR(P.FECHA_PAGO,'YYYYMM') BETWEEN $periodo_inicial AND $periodo_final
                        $where
                        UNION   all
        ...
        function getCantidadFacturas($periodo,$ruta,$sector){
            ...
            $sql="SELECT COUNT(F.CONSEC_FACTURA)
                  FROM SGC_TT_FACTURA F,SGC_TT_INMUEBLES I
                  WHERE
                      f.INMUEBLE = i.CODIGO_INM AND
                      F.FEC_EXPEDICION IS NOT NULL AND
