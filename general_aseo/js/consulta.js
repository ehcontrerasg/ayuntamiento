...
function llenarSelPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#busGenSelPro').append(new Option('Ayuntamiento:', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#busGenSelPro').append(new Option(json[x]["DESC_PROYECTO"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyServ(){
    var parametros=[];
    parametros.push({name: 'tip', value: 'flexyServAseo'});
    parametros.push({name: 'inm', value: codCol});
...
function eventoFactura(){
    ...
    if(valorrdbutton=='M'){
        document.getElementById("ifpdf").data="../../facturacion/clases/classFacturaAseoPdf.php?factura="+facCol;
    }else{
...
function reclamoPdf(id) { // Traer la fila seleccionada
    popup("../../facturacion/vistas/vista.documento_pqr_aseo.php?codigo_pqr="+id,1100,800,'yes','pop2');
}
