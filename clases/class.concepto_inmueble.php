...
/*  CREADO POR JESUS GUTIERREZ								*/
/*  FECHA CREACION 11/07/2024								*/
/********************************************************************/
...
class Concepto_inmueble extends ConexionClass{
    ...
    public function Todos ($where,$sort,$start,$end,$inmueble){
        $resultado = oci_parse($this->_db,"SELECT * FROM ( SELECT /*+ FIRST_ROWS(n) */ a.*, rownum rnum FROM
				...
				WHERE CON.COD_INMUEBLE='$inmueble' AND CON.COD_SERVICIO=CON2.COD_SERVICIO AND CON.COD_SERVICIO = 5
    ...
    public function CantidadRegistros ($fname,$tname,$where,$sort){
        ...
        $resultado = oci_parse($this->_db,"SELECT count($fname) CANTIDAD FROM $tname WHERE CON.COD_SERVICIO=CON2.COD_SERVICIO AND CON.COD_SERVICIO = 5 $where $sort");
        $banderas=oci_execute($resultado);
        if($banderas==TRUE)
        {
            oci_close($this->_db);
