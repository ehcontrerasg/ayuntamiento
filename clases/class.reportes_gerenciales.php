...
/////////////////////************************************/////////////////////////
    ...
    public function getPeriodosRango($perini, $perfin){

        $sql = "
          SELECT 
                P.ID_PERIODO FROM SGC_TP_PERIODOS P
          WHERE 
                P.ID_PERIODO>=$perini AND 
                P.ID_PERIODO<=$perfin
    ...
    public function getHistFactPerSec($proyecto, $periodo,$gerencia){
        $sql = "
        select
            F.SECTOR, F.FACTURAS, F.FACTURADO
            from SGC_TH_FACT_PERIODO_SECTOR f
            WHERE F.PROYECTO='$proyecto' AND
              F.PERIODO=$periodo  AND
              F.GERENCIA LIKE '%$gerencia%'
            order by F.SECTOR
        ";
        //echo $sql;
        $resultado = oci_parse($this->_db,$sql);
    ...
    public function getHistRecPerSec($proyecto, $periodo,$gerencia){
        $sql = "
        select
            F.SECTOR, F.PAGOS  FACTURAS, F.RECAUDADO FACTURADO
            from SGC_TH_REC_PERIODO_SECTOR f
            WHERE F.PROYECTO='$proyecto' AND
              F.PERIODO=$periodo  AND
              F.GERENCIA LIKE '%$gerencia%'
            order by F.SECTOR
    ...
    public function getHistFactPerCon($proyecto, $periodo,$gerencia){
        $sql = "
        select
        F.CONCEPTO,CH.DESCRIPCION, SUM(F.FACTURADO) FACTURADO
        from SGC_TH_FACT_CONCEPTO_GEN f, SGC_TP_CONCEPTOS_HISTORICO ch
        WHERE F.PROYECTO='$proyecto' AND
          F.PERIODO=$periodo  AND
          F.GERENCIA LIKE '%$gerencia%' AND
          CH.CODIGO=F.CONCEPTO
        
        GROUP BY F.CONCEPTO, CH.DESCRIPCION
        ORDER BY  F.CONCEPTO
        ";
        //echo $sql;
        $resultado = oci_parse($this->_db,$sql);
  ...
  public function getHistRecPerCon($proyecto, $periodo,$gerencia){
        $sql = "
        select 
        F.CONCEPTO,CH.DESCRIPCION, SUM(F.RECAUDADO) FACTURADO
        from SGC_TH_REC_CONCEPTO_GEN f, SGC_TP_CONCEPTOS_HISTORICO ch
        WHERE F.PROYECTO='$proyecto' AND
          F.PERIODO=$periodo  AND
          F.GERENCIA LIKE '%$gerencia%' AND
          CH.CODIGO=F.CONCEPTO
    ...
    public function getHistFactPerConMed($proyecto, $periodo,$gerencia){
        $sql = "
        select
        F.CONCEPTO,CH.DESCRIPCION, SUM(F.FACTURADO) FACTURADO
        from SGC_TH_FACT_CONC_MED_GEN f, SGC_TP_CONCEPTOS_HISTORICO ch
        WHERE F.PROYECTO='$proyecto' AND
          F.PERIODO=$periodo  AND
          F.GERENCIA LIKE '%$gerencia%' AND
          CH.CODIGO=F.CONCEPTO
    ...
    public function getHistRecPerConMed($proyecto, $periodo,$gerencia){
        $sql = "
        select
        F.CONCEPTO,CH.DESCRIPCION, SUM(F.RECAUDADO) FACTURADO
        from SGC_TH_REC_CONCEPTO_GEN f, SGC_TP_CONCEPTOS_HISTORICO ch
    ...
    public function getHistFactPerConAgrup($proyecto, $periodo,$gerencia){
        $sql = "
       select ROWNUM CONCEPTO, DESCRIPCION, FACTURADO from (

select
   F.CONCEPTO DESCRIPCION, SUM(F.FACTURADO) FACTURADO
from ACEASOFT.SGC_TH_FACT_PERIODO_CONCEPTO f
WHERE F.PROYECTO='$proyecto' AND
      F.PERIODO=$periodo  AND
F.GERENCIA LIKE '%$gerencia%'
...
ORDER BY  F.CONCEPTO)
    ...
    public function getHistRecPerConAgrup($proyecto, $periodo,$gerencia){
        $sql = "
       select ROWNUM CONCEPTO, DESCRIPCION, FACTURADO from (

select
   F.CONCEPTO DESCRIPCION, SUM(F.RECAUDADO) FACTURADO
from SGC_TH_REC_PERIODO_CONCEPTO f
WHERE F.PROYECTO='$proyecto' AND
      F.PERIODO=$periodo  AND
F.GERENCIA LIKE '%$gerencia%'
...
ORDER BY  F.CONCEPTO)
    ...
    public function getHistFactPerUso($proyecto, $periodo,$gerencia){
        $sql = "
        select
        F.USO,SUM(F.FACTURAS) FACTURAS, SUM(F.FACTURADO) FACTURADO
        from SGC_TH_FACT_PERIODO_USO f
        WHERE F.PROYECTO='$proyecto' AND
          F.PERIODO=$periodo  AND
          F.GERENCIA LIKE '%$gerencia%' 
    ...
    public function getHistRecPerUso($proyecto, $periodo,$gerencia){
        $sql = "
        select
        F.USO,SUM(F.PAGOS) FACTURAS, SUM(F.RECAUDADO) FACTURADO
        from SGC_TH_REC_PERIODO_USO f
        WHERE F.PROYECTO='$proyecto' AND
          F.PERIODO=$periodo  AND
          F.GERENCIA LIKE '%$gerencia%' 
