...
class Reconexion extends ConexionClass{
    ...
    public function getRecByInmFlexy ($where,$sort,$start,$end)
    {
        $sql="SELECT *
				FROM (
					...
					FROM (
						select * from (
						SELECT  TO_CHAR(RR.FECHA_PLANIFICACION,'DD/MM/YYYY HH24:MI') FECHA_PLANIFICACION ,TO_CHAR(RR.FECHA_EJE,'YYYY/MM/DD HH24:MI') FECHA_EJE,RR.TIPO_RECONEXION,RR.OBS_GENERALES,TO_CHAR(RR.FECHA_ACUERDO,'DD/MM/YYYY HH24:MI') FECHA_ACUERDO,U.LOGIN FROM SGC_tT_REGISTRO_RECONEXION RR,
                        SGC_TT_USUARIOS U
    ...
    public function setAsignaReco($coduser,$operario_asignado,$sector,$ruta,$fecini,$fecfin,$tipo,$fecPla){
        ...
        echo $sql="BEGIN SGC_P_ASIGNA_RECONEXION('$coduser','$operario_asignado','$sector','$ruta',$fecini,$fecfin,'$tipo',$fecPla,:PCODRESULT,:PMSGRESULT);COMMIT;END;";
        $resultado=oci_parse($this->_db,$sql);
    ...
    public function getValRecByByInm($inm){
        ...
        $sql="SELECT
              ...
              WHERE
                ...
                TR.PROYECTO=I.ID_PROYECTO AND
                CONCAT(I.ID_SECTOR,I.ID_RUTA) NOT IN (
    SELECT CONCAT(RC.ID_SECTOR,RC.ID_RUTA) FROM SGC_TP_RUT_EXC_CORTE RC WHERE 
      RC.FECHA_ANULACION IS NULL
  )
              ";
    ...
    public function getTiempoRecMesByPerPro($periodo,$pro,$contratista){
    ...
    public function getRecAnulMesByPerPro($periodo,$pro,$contratista){
        $sql="SELECT
                ...
                FROM SGC_TT_REGISTRO_RECONEXION RC, SGC_TT_INMUEBLES INM,SGC_TT_USUARIOS U
                WHERE RC.USR_EJE=U.ID_USUARIO
                AND U.CONTRATISTA='$contratista'
                AND RC.ANULADO='S'
    ...
    public function getRecPendMesByPerPro($periodo,$proy,$contratista){
    ...
    public function getCantRecPorDiaByPerPro($periodo,$pro,$contratista){
        $sql="  SELECT
                   TO_DATE(TO_CHAR(RC.FECHA_EJE,'YYYYMMDD'),'YYYYMMDD') FECHA_EJE,
                    COUNT(1) CANTIDAD
                FROM
                    SGC_TT_REGISTRO_RECONEXION RC,sgc_tt_INMUEBLES I, SGC_TT_USUARIOS U
                WHERE RC.USR_EJE= U.ID_USUARIO
                AND U.CONTRATISTA='$contratista'
                    AND RC.ANULADO='N'
    ...
    {
        $sql="SELECT DISTINCT FC.URL_FOTO, U.LOGIN, C.FECHA_EJE
        FROM SGC_TT_FOTOS_RECONEXION FC,SGC_TT_REGISTRO_RECONEXION C, SGC_TT_USUARIOS U
        WHERE  U.ID_USUARIO = C.USR_EJE
    ...
    public function getRecXFecAsig($fecIni, $fecFin, $proy, $cont)
    {
        ...
        $sql = "select rr.ID_INMUEBLE,rr.FECHA_EJE,rr.TIPO_RECONEXION,rr.FECHA_PLANIFICACION,
  usu.LOGIN
from SGC_TT_REGISTRO_RECONEXION rr, SGC_TT_INMUEBLES inm,
  SGC_TT_USUARIOS usu
where rr.ID_INMUEBLE=inm.CODIGO_INM and
  inm.ID_PROYECTO='$proy' AND
  USU.ID_USUARIO(+)=RR.USR_EJE AND
  USU.CONTRATISTA=$cont AND
      RR.FECHA_PLANIFICACION BETWEEN TO_DATE('$fecIni 00:00:00','YYYY-MM-DD HH24/MI/SS') AND
    ...
    public function getValRecByByInmConsGen($inm)
    {
        $inm = addslashes($inm);
        $sql = "SELECT TO_CHAR(ROUND(TR.VALOR_TARIFA))VALOR_TARIFA
              FROM
                SGC_TP_TARIFAS_RECONEXION TR,
                SGC_TT_INMUEBLES I,
                SGC_TT_MEDIDOR_INMUEBLE MI,
    ...
    SELECT CONCAT(RC.ID_SECTOR,RC.ID_RUTA) FROM SGC_TP_RUT_EXC_CORTE RC WHERE 
      RC.FECHA_ANULACION IS NULL)  
      UNION
SELECT CASE WHEN CODIGO_INM IS NOT NULL THEN 'EXENTO DE RECONEXION' END VALOR_TARIFA
FROM SGC_TT_INMUEBLES I
WHERE CODIGO_INM='$inm' AND CONCAT(I.ID_SECTOR,I.ID_RUTA)   IN (
                                                               SELECT CONCAT(RC.ID_SECTOR,RC.ID_RUTA) FROM SGC_TP_RUT_EXC_CORTE RC WHERE
                                                                   RC.FECHA_ANULACION IS NULL)";
        $resultado=oci_parse($this->_db,$sql);
        $bandera=oci_execute($resultado);
    ...
    public function getRendRecGerenciaByProyContFec($proy, $cont, $fechaIni, $fechaFin)
    {
        $sql = "
               SELECT  USU,
              (SELECT COUNT(1) FROM SGC_TT_REGISTRO_RECONEXION RC2,SGC_TT_USUARIOS U2, SGC_TT_INMUEBLES  i2
              ...
              DIAS
                ...
                SELECT
                  ...
                  U.LOGIN USU
                          ...
                          FROM SGC_TT_REGISTRO_RECONEXION RC,SGC_TT_USUARIOS U, SGC_TT_INMUEBLES I
