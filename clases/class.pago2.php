...
class Pago extends ConexionClass
{
    ...
    public function obtenerRepDetxCon($proyecto,$concepto,$fechaIn,$fechaFn)
    {
        $sql = "SELECT R.CODIGO ID_PAGO, TO_CHAR(R.FECHA_PAGO,'DD/MM/YYYY')FECPAGO, TO_CHAR(R.FECHA,'DD/MM/YYYY')fereg, NVL(DECODE(R.ESTADO,'A','Pend Aplic'),201802)ESTADO,
...
WHERE I.CODIGO_INM = R.INMUEBLE
      ...
      AND R.FECHA_PAGO BETWEEN TO_DATE('$fechaIn','DD/MM/YYYY HH24:MI:SS')
      AND TO_DATE('$fechaFn','DD/MM/YYYY HH24:MI:SS')";
        //echo $sql;
        $resultado = oci_parse(
            $this->_db, $sql
        );
    ...
    public function GerPagCorGroupSecPerBySecPer($periodo, $sec)
    {

        $sql = "SELECT COUNT(1)CANTIDAD
                        FROM SGC_TT_PAGOS P, SGC_TP_CAJAS_PAGO C, SGC_TP_ENTIDAD_PAGO E, SGC_TP_PUNTO_PAGO R, SGC_TT_INMUEBLES 
    ...
    public function getMedioPagoByPago($id_pago)
    {
    ...
    {
        $sql = "SELECT B.CODIGO,B.DESCRIPCION FROM SGC_TP_BANCOS B WHERE B.ESTADO='A'";
