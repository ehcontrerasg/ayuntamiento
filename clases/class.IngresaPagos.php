...
class IngresaPagos extends ConexionClass{
    ...
    public function datosDeudaInmueble($cod_inmueble){
		...
		NVL((SELECT SUM(F.TOTAL-F.TOTAL_PAGADO+F.TOTAL_DEBITO-F.TOTAL_CREDITO) FROM SGC_TT_FACTURA_ASEO F WHERE I.CODIGO_INM = F.INMUEBLE AND F.FACTURA_PAGADA = 'N' AND FEC_EXPEDICION IS NOT NULL),0)DEUDA,
