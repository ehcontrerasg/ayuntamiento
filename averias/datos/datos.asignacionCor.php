...
if($tipo=="genRut")
{
    ...
    $datos = $q->getCortAbiByPerZonFacMontUsoUni($pro,$zon,$facVencIni,$facVencFin,$deudaIni,$deudaFin,$uso,$uniIni,$uniFin,$calibre);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
        $zonas[$i]=$row;
        $i++;
...
if($tipo=="asig")
{
    ...
    $datos = $q->asignaCorteByZonRutOperFacDeuUsoUniMed($zon,$rut,$usu,$cod,$facIni,$facFin,$deudaIni,$deudaFin,$uso,$uniIni,$uniFin,$med,$fech,$usuViejo);
    if($datos){
