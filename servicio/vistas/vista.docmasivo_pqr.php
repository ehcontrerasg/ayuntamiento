...
if ($verificarPermisos==true):
    ...
    $pdf = new FPDF();
    $pdf->AliasNbPages();
    $pdf->title = "Reclamos Servicio al Cliente";

    $c = new Pqr();
    $resultado = $c->generaDocMasivoPqrServicio ($area_user,$tipo_pqr,$proyecto,$secini,$secfin,$rutini,$rutfin,$fecini,$fecfin,$cod_inmueble,$departamento,$motivo_pqr,$codpqr);
    while (oci_fetch($resultado)) {
        ...
        $pdf->Ln(4);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(95,5,utf8_decode("Descripción: "),0,0,'L');
        $pdf->SetFont('Arial','B',8);
