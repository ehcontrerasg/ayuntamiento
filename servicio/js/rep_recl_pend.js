...
$(document).ready(function(){
    ...
    compSession(llenarSpinerMot);


    $('#repRecPendForm').submit(
        function(){
...
function generaImp(){
    var datos=$("#repRecPendForm").serializeArray();
    $.ajax
    ({
        url : '../reportes/reporte.rep_recl_pend_exc.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.rep_recl_pend.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {

            $('#repRecPendSelPro').empty();
            $('#repRecPendSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repRecPendSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function llenarSpinerMot()
{
    ...
    ({
        ...
        success : function(json) {
            ...
            $('#repRecPendSelMot').empty();
            $('#repRecPendSelMot').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repRecPendSelMot').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
