...
if ($acueducto == 'SD') {
    ...
    $pdf->Text(60, 15, utf8_decode("CORPORACIÓN DEL ACUEDUCTO Y ALCANTARILLADO DE SANTO DOMINGO"));
    $pdf->Text(115, 22, utf8_decode("CAASD"));
    $pdf->Image("../../images/LogoCaasd.jpg", 10, 10, 30, 30);
    //$pdf->Image("../../images/logo_caasd_nuevo.png", 10, 10, 30, 30);
}
if ($acueducto == 'BC') {
    $ciudad = "Boca Chica";
    $pdf->Text(60, 15, utf8_decode("CORPORACIÓN DEL ACUEDUCTO Y ALCANTARILLADO DE BOCACHICA "));
    $pdf->Text(115, 22, utf8_decode("CORAABO"));
    $pdf->Image("../../images/logo_coraabo.jpg", 10, 10, 30, 30);
}
$pdf->SetFont('arial', "B", 12);
$pdf->Text(90, 33, utf8_decode("DESCUENTO PLAN PONTE AL DÍA"));
$pdf->SetFont('arial', "B", 10);
...
$pdf->SetFont('arial', "", 9);
if($acueducto == 'SD') {
    $pdf->Text(70, 74, utf8_decode(", y la CAASD suscriben el presente acuerdo de pago por los servicios prestados de esta."));
}
else{
    $pdf->Text(70, 74, utf8_decode(", y CORAABO suscriben el presente acuerdo de pago por los servicios prestados de esta."));
}
...
$pdf->SetFont('arial', "", 9);
if($acueducto == 'SD') {
    $pdf->Text(25, 85, utf8_decode("El Deudor declara que adeuda a la CAASD la suma de"));
}
else{
    $pdf->Text(25, 85, utf8_decode("El Deudor declara que adeuda a CORAABO la suma de"));
}
...
while(oci_fetch($datos)) {
    ...
    $codserv = oci_result($datos, "COD_SERVICIO");
    ...
    if($concepto <> 'Acometida' && $concepto <> 'Presupuestos' && $concepto <> 'Carg. Cheque Dev.' && $concepto <> 'Cargos por RI'){
        $pdf->SetTextColor(255,0,0);

        $f=new facturas();
        $datosf=$f->estcon4($inmueble, $descuento, $codserv);
        while(oci_fetch($datosf)) {
            $conceptod = oci_result($datosf, "CONCEPTOD");
            $codservd = oci_result($datosf, "COD_SERVICIOD");
            $desconcep = oci_result($datosf, "VALORD");


        //$desconcep = round(($valor * $descuento)/100);
        $desconcepf = number_format($desconcep,2,',','.');
        $pdf->SetY(98 + $posy); $pdf->SetX(113);
        $pdf->Cell(10, 5, utf8_decode('RD$ '), 0, 3, 'C', false);
        $pdf->SetY(98 + $posy); $pdf->SetX(122);
        $pdf->Cell(22, 5, $desconcepf, 0, 3, 'R', false);
        $pagara = $valor - $desconcep;
        ...
        }oci_free_statement($datosf);
    }
    else{
        $pdf->SetY(98 + $posy); $pdf->SetX(161);
        $pdf->Cell(10, 5, utf8_decode('RD$ '), 0, 3, 'C', false);
        $pdf->SetY(98 + $posy); $pdf->SetX(170);
        $pdf->Cell(22, 5, 0.00, 0, 3, 'R', false);
    }
    $posy += 6;
    $deudatotal += $valor;
    $totalpagar += $pagara;
...
$reduccion = 'RD$ '. $totalpagarf;
if($acueducto == 'SD') {
    $pdf->SetFont('arial', "", 9);
    $pdf->SetY(98 + $posy); $pdf->SetX(27);
    $pdf->Cell(10, 5, utf8_decode('La CAASD acuerda reducir la deuda a '), 0, 3, 'L', false);
    $pdf->SetFont('arial', "B", 9);
    $pdf->SetY(98 + $posy); $pdf->SetX(97);
    $pdf->Cell(10, 5, utf8_decode($reduccion), 0, 3, 'L', false);
}
else{
    $pdf->SetFont('arial', "", 9);
    $pdf->SetY(98 + $posy); $pdf->SetX(27);
    $pdf->Cell(10, 5, utf8_decode('CORAABO  acuerda reducir la deuda a '), 0, 3, 'L', false);
    $pdf->SetFont('arial', "B", 9);
    $pdf->SetY(98 + $posy); $pdf->SetX(97);
    $pdf->Cell(10, 5, utf8_decode($reduccion), 0, 3, 'L', false);
}
...
$valorCuota = round($totalpagar / $cuotas);
$cuotaInicial = $valorCuota;
$cuotasfinal = $cuotas - 1;
if ($cuotasfinal == 0) $valorCuota = '0';
...
$pdf->SetY(98 + $posy); $pdf->SetX(170);
$pdf->Cell(20, 5, utf8_decode($cuotaInicialf), 'B', 3, 'C', false);
$pdf->SetFont('arial', "", 9);
$pdf->SetTextColor(0,0,0);
...
if ($cuotasfinal == 1) $descuota = 'cuota';
...
$pdf->SetFont('arial', "B", 9);
$pdf->SetTextColor(255,0,0);
$pdf->SetY(98 + $posy); $pdf->SetX(48);
$pdf->Cell(10, 5, utf8_decode('RD$ '), 'B', 3, 'C', false);
$pdf->SetY(98 + $posy); $pdf->SetX(58);
$pdf->Cell(15, 5, utf8_decode($reconexionf), 'B', 3, 'C', false);
$pdf->SetFont('arial', "", 9);
$pdf->SetTextColor(0,0,0);
$pdf->SetY(98 + $posy); $pdf->SetX(75);
$pdf->Cell(10, 5, utf8_decode('y  lo  restante  mediante  '.$cuotasfinal.'  '.$descuota.' de'), 0, 3, 'L', false);
$pdf->SetFont('arial', "B", 9);
$pdf->SetY(98 + $posy); $pdf->SetX(131);
$pdf->Cell(10, 5, utf8_decode('RD$ '), 'B', 3, 'C', false);
$pdf->SetY(98 + $posy); $pdf->SetX(141);
$pdf->Cell(15, 5, utf8_decode($valorCuotaf), 'B', 3, 'C', false);
$pdf->SetFont('arial', "", 9);
$pdf->SetY(98 + $posy); $pdf->SetX(159);
$pdf->Cell(50, 5, utf8_decode(' cancelada   dicha  deuda  de '), 0, 3, 'L', false);
$posy += 6;
$pdf->SetY(98 + $posy); $pdf->SetX(10);
$pdf->Cell(50, 5, utf8_decode('conformidad con los  pagares firmados, mas la  factura del mes, mas los  nuevos cargos  mensuales que se produzcan en razón de los '), 0, 3, 'L', false);
$posy += 6;
$pdf->SetY(98 + $posy); $pdf->SetX(10);
$pdf->Cell(50, 5, utf8_decode('servicios prestados.'), 0, 3, 'L', false);
$posy += 6;
$pdf->SetFont('arial', "B", 9);
$pdf->SetY(98 + $posy); $pdf->SetX(10);
...
$pdf->Cell(10, 5, utf8_decode('El  incumplimiento  de  cualquiera de  las cuotas  del  acuerdo  de pago, hará  perder el  beneficio  concedido  vía  amnistía y'), 0, 3, 'L', false);
$posy += 6;
$pdf->SetY(98 + $posy); $pdf->SetX(10);
$pdf->Cell(10, 5, utf8_decode('devolverá la deuda al estado original que se  encontraba al momento de la solicitud del presente acuerdo, sin perjuicio de las acciones'), 0, 3, 'L', false);
$posy += 6;
$pdf->SetY(98 + $posy); $pdf->SetX(10);
$pdf->Cell(10, 5, utf8_decode('legales que sean pertinentes. En el caso que se aplique un único pago, no se aplicará dicha medida.'), 0, 3, 'L', false);
$posy += 10;
$totalnow = $cuotaInicial + $reconexion;
$totalnowf = number_format($totalnow,2,',','.');
$pdf->SetFont('arial', "B", 9);
$pdf->SetY(98 + $posy); $pdf->SetX(10);
...
$pdf->Cell(15, 5, utf8_decode($totalnowf), 0, 3, 'R', false);
$posy += 10;
$pdf->SetFont('arial', "", 9);
$pdf->SetY(98 + $posy); $pdf->SetX(10);
...
$pdf->SetY(98 + $posy); $pdf->SetX(97);
if ($acueducto == 'SD') {
    $pdf->Cell(60, 5, utf8_decode('Por la CAASD'), 'T', 3, 'C', false);
}
else{
    $pdf->Cell(60, 5, utf8_decode('Por CORAABO'), 'T', 3, 'C', false);
}
...
$posy += 4;
$pdf->SetY(98 + $posy); $pdf->SetX(97);
$pdf->Cell(60, 5, utf8_decode(/*$docasiste*/), 0, 3, 'C', false);
...
$pdf->SetY(98 + $posy); $pdf->SetX(10);
if ($acueducto == 'SD') {
    $pdf->Cell(15, 5, utf8_decode('Original CAASD acompañado del estado de cuenta. Duplicado al usuario. '), 0, 3, 'L', false);
}
else{
    $pdf->Cell(15, 5, utf8_decode('Original CORAABO acompañado del estado de cuenta. Duplicado al usuario. '), 0, 3, 'L', false);
}

$posy += 5;
$pdf->SetFont('arial', "I", 7);
$pdf->SetY(98 + $posy); $pdf->SetX(20);
$pdf->Cell(15, 5, utf8_decode('Código: FO-SEC-40 '), 0, 3, 'L', false);
$pdf->SetY(98 + $posy); $pdf->SetX(50);
$pdf->Cell(15, 5, utf8_decode('Edición No.:01'), 0, 3, 'L', false);

///DESDE ACA GENERA LA SOLICITUD

/*$d = new PQRs();
$resultado = $d->getDatByPqrAmnistia ($inmueble);
...
while (oci_fetch($resultado)) {
    ...
    if ($acueducto == 'SD') {
        ...
        $pdf->Image("../../images/LogoCaasd.jpeg", 14, 9, 20);
    }
    ...
    if ($acueducto == 'BC') {
        ...
        $pdf->Text(92, 22, "GERENCIA COMERCIAL");
        $pdf->Text(58, 29, utf8_decode("CONSTANCIA DE RECEPCIÓN DE RECLAMOS Y SOLICITUDES"));
        $pdf->Image("../../images/logo_coraabo.jpeg", 10, 15, 40);
    }
