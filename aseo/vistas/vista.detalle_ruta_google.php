...
// Create the placemark.
        ...
        function newmap(xigeo,yigeo,cod_inm,fechafi){
            ...
            var contentString = '<div id="content"><center>'+cod_inm+'</center><p><center>fecha:'+fechafi+'</center> <p><center><a target="_blank" href="http://developermovil01.serveftp.net:8081/acea/catastro/fotos_mantenimiento.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodoid; ?>&periodoF=<?php echo $periodoF; ?>">Ver fotos</a></center></div>';
...
//x=xgeo;
            ...
            var contentString = '<div id="content"><center>'+cod_inm+'</center><p><center>fecha:'+fechafi+'</center> <p><center><a target="_blank" href="http://developermovil01.serveftp.net:8081/acea/catastro/fotos_mantenimiento.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodoid; ?>&periodoF=<?php echo $periodoF; ?>">Ver fotos</a></center><p>Orden:'+sec+'</div>';
...
//x=xgeo;
            ...
            var contentString = '<div id="content"><center>'+cod_inm+'</center><p><center>fecha:'+fechafi+'</center> <p><center><a  href="http://developermovil01.serveftp.net:8081/acea/catastro/fotos_mantenimiento.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodoid; ?>&periodoF=<?php echo $periodoF; ?>">Ver fotos</a></center></div>';
...
<form action="acea_google.php"  name="formulario" method="post" >
    ...
    $sql = "SELECT * FROM (
 SELECT LONGITUD, LATITUD, ID_INMUEBLE ,  TO_CHAR(FECHA_FIN,'DD/MM/YYYY HH24:MI:SS') FECHAFIN
 FROM SGC_TT_ASIGNACION
 WHERE CONCAT(ID_SECTOR,ID_RUTA) = $cod_ruta AND FECHA_FIN IS NOT NULL AND ID_PERIODO BETWEEN $periodoid AND $periodoF  AND LONGITUD IS NOT NULL AND LONGITUD <> '(sin_datos)'
 ...
 WHERE ROWNUM = 1";
    ...
    while (oci_fetch($stid)) {
        ...
        $id_sistema_ini = oci_result($stid, 'ID_INMUEBLE');
 ...
 WHERE ROWNUM = 1";
    ...
    while (oci_fetch($stid)) {
        $xi             = oci_result($stid, 'LONGITUD');
        $yi             = oci_result($stid, 'LATITUD');
        $id_sistema_fin = oci_result($stid, 'ID_INMUEBLE');
        $fechafin       = oci_result($stid, 'FECHAFIN');
    }
    oci_free_statement($stid);
    if (($xi != null && $xi != '(sin_datos)') || ($yi != null && $yi != '(sin_datos)')) {
    ...
    $sql = "SELECT LONGITUD, LATITUD, ID_INMUEBLE ,TO_CHAR(FECHA_FIN,'DD/MM/YYYY HH24:MI:SS') FECHAFIN, (rownum+1) SEC
 FROM SGC_TT_ASIGNACION
 WHERE CONCAT(ID_SECTOR,ID_RUTA) = $cod_ruta AND ID_PERIODO BETWEEN $periodoid AND $periodoF AND FECHA_FIN IS NOT NULL AND ID_INMUEBLE <> $id_sistema_ini AND ID_INMUEBLE <> $id_sistema_fin
 AND LONGITUD IS NOT NULL AND LONGITUD <> '(sin_datos)' ORDER BY FECHA_FIN ASC";
    $stid = oci_parse($link, $sql);
    oci_execute($stid, OCI_DEFAULT);
    $i = 1;
    while (oci_fetch($stid)) {
        ...
        $cod_sistema = oci_result($stid, 'ID_INMUEBLE');
        $fechafin    = oci_result($stid, 'FECHAFIN');
        $secq        = $i;
