...
session_start();
	...
	$l=new Pqr();
	$valores=$l->getCantPqrByProTipSecRutFecCodAreFlexy($proyecto, $tipo_pqr, $secini, $secfin, $rutini, $rutfin, $fecini, $fecfin, $cod_inmueble, $where, '10');
	while (oci_fetch($valores)) {
		$total = oci_result($valores, 'CANTIDAD');
	}oci_free_statement($valores);

	$l=new Pqr();
	$registros=$l->getDatosPqrByProTipSecRutFecInmFlexy($proyecto, $tipo_pqr, $secini, $secfin, $rutini, $rutfin, $fecini, $fecfin, $cod_inmueble, $start, $end, $where, '10');
