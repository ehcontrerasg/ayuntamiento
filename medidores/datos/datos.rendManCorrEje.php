...
if($tipo=='corrManCorrEje'){
    ...
    $datos = $l->getCoordManCorrReaByFecEjeRutSecPer($fechaini,$fechafin,$ruta,$usr);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
      //  print_r($row);
        $con[$i]=$row;
...
if($tipo=="flexy")
{
    ...
    $t=new Medidor();
    $total=$t->getCantRutManCorrEjePerEje($proyecto,$sector,$fecini,$fecfin);


    $l=new Medidor();
    $stid=$l->obtenerResumenRutasManCorrEje($sort,$start,$end,$proyecto,$sector,$fecini,$fecfin);
    ...
    while (oci_fetch($stid)) {
        ...
        if ($rc) $json .= ",";
        $json .= "\n{";
        $json .= "id:'".$cod_operario.' '.$proyecto.' '.$cod_ruta.' '.$fecini.' '.$fecfin."',";
        $json .= "cell:['<b>" .$numero."</b>'";
        $json .= ",'".addslashes($cod_operario)."'";
...
if($tipo=='flexyDetRend'){
    ...
    $l=new Medidor();
    $registros=$l->getManCorrReaByFecEjeRutSecPer($fecini,$fecfin,$ruta,$usr);
    ...
    while (oci_fetch($registros)) {
        ...
        $c=new Medidor();
        $totFot=$c->getCantFotManCorrByOrd($orden);
