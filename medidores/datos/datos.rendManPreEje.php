...
if($tipo=='corrManPreEje'){
    ...
    $datos = $l->getCoordManPreReaByFecEjeRutSecPer($fechaini,$fechafin,$ruta,$usr);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
      //  print_r($row);
        $con[$i]=$row;
...
if($tipo=="flexy")
{
    ...
    $t=new Medidor();
    $total=$t->getCantRutManPreEjePerEje($proyecto,$sector,$fecini,$fecfin);


    $l=new Medidor();
    $stid=$l->obtenerResumenRutasManPreEje($sort,$start,$end,$proyecto,$sector,$fecini,$fecfin);
...
if($tipo=='flexyDetRend'){
    ...
    $l=new Medidor();
    $registros=$l->getManPreReaByFecEjeRutSecPer($fecini,$fecfin,$ruta,$usr);
    ...
    while (oci_fetch($registros)) {
        ...
        $c=new Medidor();
        $totFot=$c->getCantFotManPreByOrd($orden);
