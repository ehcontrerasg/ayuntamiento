...
if($tipo=='corrManPre'){
    ...
    $datos = $l->getCoordManPreReaByFecRutSecPer($fecha,$ruta,$usr);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
      //  print_r($row);
        $con[$i]=$row;
...
if($tipo=="flexy")
{
    ...
    $t=new Medidor();
    $total=$t->getCantRutManPrePerEje($proyecto,$sector,$fecini,$fecfin);


    $l=new Medidor();
    $stid=$l->obtenerResumenRutasManPre($sort,$start,$end,$proyecto,$sector,$fecini,$fecfin);
...
if($tipo=='flexyDetRend'){
    ...
    $l=new Medidor();
    $registros=$l->getManPreReaByFecRutSecPer($fec,$ruta,$usr);
    ...
    while (oci_fetch($registros)) {
        ...
        $totFot = oci_result($registros, 'CANTIDAD_FOTOS');
