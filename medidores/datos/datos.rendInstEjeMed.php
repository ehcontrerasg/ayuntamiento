...
if($tipo=='corrInstEjeMed'){
    ...
    $datos = $l->getCoordInstMedReaByFecEjeRutSecPer($fechaini,$fechafin,$ruta,$usr);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
      //  print_r($row);
        $con[$i]=$row;
...
if($tipo=="flexy")
{
    ...
    $t=new Medidor();
    $total=$t->getCantRutInstMedFecEje($proyecto,$sector,$fecini,$fecfin);


    $l=new Medidor();
    $stid=$l->obtenerResumenRutasInstMedFecEje($sort,$start,$end,$proyecto,$sector,$fecini,$fecfin);
...
if($tipo=='flexyDetRend'){
    ...
    $l=new Medidor();
    $registros=$l->getInstMedReaByFecIniFinRutSecPer($fecini,$fecfin,$ruta,$usr);
