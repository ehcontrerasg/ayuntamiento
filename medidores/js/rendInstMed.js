...
$(document).ready(function(){
    ...
    $('#rendInstMedForm').submit(
        function(){
            compSession(flexyRendInstMed);
        }
    )
...
function generaRep(){
    ...
    if($("#rendInstMedFecIni").val().trim()!='' && $("#rendInstMedFecFin").val().trim()!='') {
        fecini = $("#rendInstMedFecIni").val();
        fecfin = $("#rendInstMedFecFin").val();

        var datos = $("#rendInstMedForm").serializeArray();
        $.ajax
        ({
            url: '../reportes/reporte.rendInstMed.php?fecini='+fecini+'&fecfin='+fecfin,
            type: 'POST',
            dataType: 'text',
            data: datos,
...
function initMap() {
    ...
    parametros.push({name: 'tip', value: 'corrInstMed'});

    $.ajax
    ({
        url : '../datos/datos.rendInstMed.php',
        type : 'POST',
        dataType : 'json',
        data : parametros,
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#rendInstMedSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendInstMedSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function cargSectores()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selSec' , pro:$('#rendInstMedSelPro').val() },
        success : function(json) {
            $('#rendInstMedSelSec').empty();
            $('#rendInstMedSelSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendInstMedSelSec').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyRendInstMed(){

    var parametros =
        $("#rendInstMedForm").serializeArray();
        parametros.push({name: 'tip', value: 'flexy'});
        parametros.push({name: 'fecIni', value: $("#rendInstMedFecIni").val()});
        parametros.push({name: 'fecFin', value: $("#rendInstMedFecFin").val()});
        parametros.push({name: 'tip', value: 'flexy'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendInstMed").flexigrid
    (
        {
            url: '../datos/datos.rendInstMed.php',
            dataType: 'json',
    ...
    $("#flexRendInstMed").flexOptions({url: '../datos/datos.rendInstMed.php'});
    $("#flexRendInstMed").flexOptions({params: parametros});
    $("#flexRendInstMed").flexReload();
}



function flexRendInstMedDet(){
    ...
    $("#flexRendInstMedDet").flexOptions({url: '../datos/datos.rendInstMed.php'});
    $("#flexRendInstMedDet").flexOptions({title: 'Detalle Instalaciones Operario ruta:'+ruta});
    $("#flexRendInstMedDet").flexOptions({params: parametros});
    $("#flexRendInstMedDet").flexReload();
}



function asigevenRend() {
    tabflexPag = document.getElementById("flexRendInstMed");
...
function eventoPagos(){
    ...
    compSession(flexRendInstMedDet);
    initMap();
