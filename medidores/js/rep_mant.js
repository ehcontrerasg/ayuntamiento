...
function generaRep(){
    var datos=$("#repManMedForm").serializeArray();

    $.ajax
    ({
        url : '../reportes/reporte.rep_mant.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.rep_mant.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {
            $('#repManMedSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repManMedSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
