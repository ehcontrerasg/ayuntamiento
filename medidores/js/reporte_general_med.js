...
function generaRep(){
    var datos=$("#repGenMedForm").serializeArray();
    $.ajax
    ({
        url : '../reportes/reporte.General_medidores.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.reporte_general_med.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {
            $('#repGenMedSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repGenMedSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
