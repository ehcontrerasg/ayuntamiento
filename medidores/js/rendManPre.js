...
$(document).ready(function(){
    ...
    $('#rendManPreForm').submit(
        function(){
            compSession(flexyRendManPre);
        }
    )
...
function initMap() {
    ...
    parametros.push({name: 'tip', value: 'corrManPre'});

    $.ajax
    ({
        url : '../datos/datos.rendManPre.php',
        type : 'POST',
        dataType : 'json',
        data : parametros,
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#rendManPreSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendManPreSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function cargSectores()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selSec' , pro:$('#rendManPreSelPro').val() },
        success : function(json) {
            $('#rendManPreSelSec').empty();
            $('#rendManPreSelSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendManPreSelSec').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyRendManPre(){

    var parametros =
        $("#rendManPreForm").serializeArray();
        parametros.push({name: 'tip', value: 'flexy'});
        parametros.push({name: 'fecIni', value: $("#rendManPreFecIni").val()});
        parametros.push({name: 'fecFin', value: $("#rendManPreFecFin").val()});
        parametros.push({name: 'tip', value: 'flexy'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendManPre").flexigrid
    (
        {
            url: '../datos/datos.rendManPre.php',
            dataType: 'json',
    ...
    $("#flexRendManPre").flexOptions({url: '../datos/datos.rendManPre.php'});
    $("#flexRendManPre").flexOptions({params: parametros});
    $("#flexRendManPre").flexReload();
}



function flexRendManPreDet(){
    ...
    $("#flexRendManPreDet").flexOptions({url: '../datos/datos.rendManPre.php'});
    $("#flexRendManPreDet").flexOptions({title: 'Detalle Mantenimientos Operario ruta:'+ruta});
    $("#flexRendManPreDet").flexOptions({params: parametros});
    $("#flexRendManPreDet").flexReload();
}



function asigevenRend() {
    tabflexPag = document.getElementById("flexRendManPre");
