...
$(document).ready(function(){
    ...
    $('#rendInstEjeMedForm').submit(
        function(){
            compSession(flexyRendInstEjeMed);
        }
    )
...
function generaRep(){
    ...
    if($("#rendInstEjeMedFecIni").val().trim()!='' && $("#rendInstEjeMedFecFin").val().trim()!='') {
        fecini = $("#rendInstEjeMedFecIni").val();
        fecfin = $("#rendInstEjeMedFecFin").val();

        var datos = $("#rendInstEjeMedForm").serializeArray();
        $.ajax
        ({
            url: '../reportes/reporte.rendInstEjeMed.php?fecini='+fecini+'&fecfin='+fecfin,
            type: 'POST',
            dataType: 'text',
            data: datos,
...
function initMap() {
    ...
    parametros.push({name: 'fecini', value: fecini});
    parametros.push({name: 'fecfin', value: fecfin});
    parametros.push({name: 'ruta', value: ruta});
    parametros.push({name: 'usr', value: usuario});
    parametros.push({name: 'tip', value: 'corrInstEjeMed'});

    $.ajax
    ({
        url : '../datos/datos.rendInstEjeMed.php',
        type : 'POST',
        dataType : 'json',
        data : parametros,
    ...
    ({
        ...
        success : function(json) {
            if(json==true){
                ...
                $("#rendInstEjeMedSelPro").focus(false);
                swal({
                        title: "Mensaje ",
                        html: true,
                        type: "input",
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#rendInstEjeMedSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendInstEjeMedSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function cargSectores()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selSec' , pro:$('#rendInstEjeMedSelPro').val() },
        success : function(json) {
            $('#rendInstEjeMedSelSec').empty();
            $('#rendInstEjeMedSelSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendInstEjeMedSelSec').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyRendInstEjeMed(){
    console.log('entro a la funcion');
    var parametros =
        $("#rendInstEjeMedForm").serializeArray();
        parametros.push({name: 'tip', value: 'flexy'});
        parametros.push({name: 'fecIni', value: $("#rendInstEjeMedFecIni").val()});
        parametros.push({name: 'fecFin', value: $("#rendInstEjeMedFecFin").val()});
        parametros.push({name: 'tip', value: 'flexy'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendInstEjeMed").flexigrid
    (
        {
            url: '../datos/datos.rendInstEjeMed.php',
            dataType: 'json',
    ...
    $("#flexRendInstEjeMed").flexOptions({url: '../datos/datos.rendInstEjeMed.php'});
    $("#flexRendInstEjeMed").flexOptions({params: parametros});
    $("#flexRendInstEjeMed").flexReload();
}



function flexRendInstEjeMedDet(){
    ...
    proyecto=variables[1];
    ruta=variables[2];
    fechaini=variables[3];
    fechafin=variables[4];
    var parametros=[];

    parametros.push({name: 'fecini', value: fechaini});
    parametros.push({name: 'fecfin', value: fechafin});
    parametros.push({name: 'ruta', value: ruta});
    parametros.push({name: 'usr', value: usuario});
    ...
    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendInstEjeMedDet").flexigrid
    (
        {
            url: '../datos/datos.rendInstEjeMed.php',
            dataType: 'json',
            ...
            colModel :
                [
                    ...
                    {display: 'Fecha Instalacion',  width: 89, sortable: false, align: 'center'},
                    {display: 'Fotos',  width: 54, sortable: false, align: 'center'}
                ],
            usepager: true,
            title: 'Detalle Instalaciones Operario ruta' ,
            useRp: false,
            page: 1,
            showTableToggleBtn: false,
            width: 1000,
    ...
    $("#flexRendInstEjeMedDet").flexOptions({url: '../datos/datos.rendInstEjeMed.php'});
    $("#flexRendInstEjeMedDet").flexOptions({title: 'Detalle Instalaciones Operario ruta:'+ruta});
    $("#flexRendInstEjeMedDet").flexOptions({params: parametros});
    $("#flexRendInstEjeMedDet").flexReload();
}



function asigevenRend() {
    tabflexPag = document.getElementById("flexRendInstEjeMed");
...
function eventoPagos(){
    ...
    compSession(flexRendInstEjeMedDet);
    initMap();
...
function fotos(id) { // Traer la fila seleccionada
    ...
    popup("vista.fotos_InstMed.php?orden="+id,1100,800,'yes');
}
