...
$(document).ready(function(){
    ...
    $('#rendManCorrEjeForm').submit(
        function(){
            compSession(flexyRendManCorrEje);
        }
    )
...
function initMap() {
    ...
    parametros.push({name: 'tip', value: 'corrManCorrEje'});

    $.ajax
    ({
        url : '../datos/datos.rendManCorrEje.php',
        type : 'POST',
        dataType : 'json',
        data : parametros,
    ...
    ({
        ...
        success : function(json) {
            if(json==true){
                ...
                $("#rendManCorrEjeSelPro").focus(false);
                swal({
                        title: "Mensaje ",
                        html: true,
                        type: "input",
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#rendManCorrEjeSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendManCorrEjeSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function cargSectores()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selSec' , pro:$('#rendManCorrEjeSelPro').val() },
        success : function(json) {
            $('#rendManCorrEjeSelSec').empty();
            $('#rendManCorrEjeSelSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendManCorrEjeSelSec').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyRendManCorrEje(){

    var parametros =
        $("#rendManCorrEjeForm").serializeArray();
        parametros.push({name: 'tip', value: 'flexy'});
        parametros.push({name: 'fecIni', value: $("#rendManCorrEjeFecIni").val()});
        parametros.push({name: 'fecFin', value: $("#rendManCorrEjeFecFin").val()});
        parametros.push({name: 'tip', value: 'flexy'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendManCorrEje").flexigrid
    (
        {
            url: '../datos/datos.rendManCorrEje.php',
            dataType: 'json',
    ...
    $("#flexRendManCorrEje").flexOptions({url: '../datos/datos.rendManCorrEje.php'});
    $("#flexRendManCorrEje").flexOptions({params: parametros});
    $("#flexRendManCorrEje").flexReload();
}



function flexRendManCorrEjeDet(){
    ...
    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendManCorrEjeDet").flexigrid
    (
        {
            url: '../datos/datos.rendManCorrEje.php',
            dataType: 'json',
    ...
    $("#flexRendManCorrEjeDet").flexOptions({url: '../datos/datos.rendManCorrEje.php'});
    $("#flexRendManCorrEjeDet").flexOptions({title: 'Detalle Mantenimientos Operario ruta:'+ruta});
    $("#flexRendManCorrEjeDet").flexOptions({params: parametros});
    $("#flexRendManCorrEjeDet").flexReload();
}



function asigevenRend() {
    tabflexPag = document.getElementById("flexRendManCorrEje");
...
function eventoPagos(){
    ...
    compSession(flexRendManCorrEjeDet);
    initMap();
