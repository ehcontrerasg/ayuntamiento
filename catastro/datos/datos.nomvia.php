...
$user_input2=strtoupper($user_input);
       	...
       	$sql = "SELECT CONSEC_NOM_VIA, DESC_NOM_VIA
		FROM SGC_TP_NOMBRE_VIA 
		WHERE DESC_NOM_VIA LIKE '$user_input2%' ";
		//echo $sql;
		$stid = oci_parse($link, $sql);
		oci_execute($stid, OCI_DEFAULT);
		while (oci_fetch($stid)) {
        
			$json_arr["id"] = oci_result($stid, 'CONSEC_NOM_VIA') ;
			$json_arr["value"] = oci_result($stid, 'CONSEC_NOM_VIA');
