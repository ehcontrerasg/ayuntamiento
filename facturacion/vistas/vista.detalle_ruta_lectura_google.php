 ...
 function newmap(xigeo,yigeo,cod_inm,fechafi){
	...
	var icon_red = '../../images/flag-red.png';	
 	...
 	var marker = new google.maps.Marker({       
 	 ...
 	 var contentString = '<div id="content"><left>Codigo Inmueble: '+cod_inm+'</left><p><left>Fecha lec: '+fechafi+'</left> <p><center><a target="_blank" href="http://developermovil01.serveftp.net:8081/acea/facturacion/vistas/vista.fotos_lectura.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodo; ?>">Ver fotos</a></center><p>Orden: 1</div>';
...
var icon_cred = '../../images/circle_red.png';	
	...
	var contentString = '<div id="content"><left>Codigo Inmueble: '+cod_inm+'</left><p><left>Fecha Lec: '+fechafi+'</left> <p><center><a target="_blank" href="http://developermovil01.serveftp.net:8081/acea/facturacion/vistas/vista.fotos_lectura.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodo; ?>">Ver fotos</a></center><p>Orden: '+sec+'</div>';
...
var icon_green = '../../images/flag-green.png';	
	...
	var contentString = '<div id="content"><left>Codigo Inmueble: '+cod_inm+'</left><p><left>Fecha Lec: '+fechafi+'</left> <p><center><a  href="http://developermovil01.serveftp.net:8081/acea/facturacion/vistas/vista.fotos_lectura.php?cod_sistema='+cod_inm+'&periodo=<?php echo $periodo; ?>">Ver fotos</a></center><p>Orden: '+secfin+'</div>';
...
<form action="acea_google.php"  name="formulario" method="post" >
	...
	<?php
		...
		if($fecini != "" && $fecfin != "") 
			$where = " AND FECHA_LECTURA_ORI BETWEEN TO_DATE('$fecini 00:00:00', 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE('$fecfin 23:59:59', 'YYYY-MM-DD HH24:MI:SS')";
	
  //traemos el primer punto leido con coordenada
 $sql="SELECT * FROM (
 SELECT LONGITUD, LATITUD, COD_INMUEBLE, TO_CHAR(FECHA_LECTURA_ORI,'DD/MM/YYYY HH24:MI:SS') FECHAFIN
 FROM SGC_TT_REGISTRO_LECTURAS
 WHERE RUTA = $cod_ruta  AND LONGITUD IS NOT NULL 
 AND LONGITUD <> '0'
 $where 
 ORDER BY FECHA_LECTURA_ORI ASC) 
 WHERE ROWNUM = 1";
 //echo $sql;
 ...
 AND LONGITUD <> '0'
 $where 
 ORDER BY FECHA_LECTURA_ORI DESC) 
 WHERE ROWNUM = 1";
 $stid = oci_parse($link, $sql);
 oci_execute($stid, OCI_DEFAULT);
 while (oci_fetch($stid)) {
 ...
 $sql="SELECT LONGITUD, LATITUD, COD_INMUEBLE ,TO_CHAR(FECHA_LECTURA_ORI,'DD/MM/YYYY HH24:MI:SS') FECHAFIN, (rownum+1) SEC
 FROM SGC_TT_REGISTRO_LECTURAS
