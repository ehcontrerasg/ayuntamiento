    ...
    public function facpend ($where,$sort,$start,$end)
    {
        $sql="SELECT *
				FROM (
					...
					FROM (
						SELECT F.CONSEC_FACTURA, (F.TOTAL-F.TOTAL_PAGADO+F.TOTAL_DEBITO-F.TOTAL_CREDITO) TOTAL , F.PERIODO, F.FEC_EXPEDICION,concat(NU.ID_NCF,F.NCF_CONSEC) ncf FROM SGC_TT_FACTURA F,sgc_tp_ncf_usos nu WHERE F.FACTURA_PAGADA='N'
						and F.NCF_ID=NU.ID_NCF_USO
						AND F.FEC_EXPEDICION IS NOT NULL
                        AND F.FECHA_PAGO IS NULL
						$where
...
////////////////// facturas pendientes deuda ///////////////////////
    ...
    public function valorDeudaAseo ($codinm)
    {
        $sql="SELECT SUM(F.TOTAL - F.TOTAL_PAGADO + F.TOTAL_DEBITO - F.TOTAL_CREDITO) DEUDA
                FROM SGC_TT_FACTURA_ASEO F
                WHERE F.FACTURA_PAGADA = 'N'
                AND F.INMUEBLE = '$codinm'";
        //echo $sql;
        $resultado = oci_parse(
    ...
    public function categoriaInmuebleAseo ($codinm)
    {
        $sql="SELECT T.CATEGORIA
                FROM SGC_TP_TARIFAS T, SGC_TT_SERVICIOS_INMUEBLES SI, SGC_TT_INMUEBLES I
                WHERE I.CODIGO_INM = SI.COD_INMUEBLE
    ...
    public function detfac ($where,$sort,$start,$end,$factura)
    {
        $sql="SELECT *
				FROM (
					SELECT /*+ FIRST_ROWS(n) */ a.*, rownum rnum
					FROM (
						select S.DESC_SERVICIO CONCEPTO,RANGO,UNIDADES,VALOR-VALOR_PAGADO+TOTAL_DEBITO-TOTAL_CREDITO VALOR,S.COD_SERVICIO
...
///////////////////////////////////////////// deuda estado por concepto//////////////////////////
    ...
    public function estcon ($where,$sort,$start,$end)
    {
        $sql="SELECT *
				FROM (
					SELECT /*+ FIRST_ROWS(n) */ a.*, rownum rnum
					FROM (SELECT S.DESC_SERVICIO CONCEPTO, SUM(NVL(DF.VALOR-DF.VALOR_PAGADO+TOTAL_DEBITO-TOTAL_CREDITO,0)) VALOR, count(S.DESC_SERVICIO) NUMFAC FROM SGC_TT_FACTURA F, SGC_TT_DETALLE_FACTURA DF, SGC_TP_SERVICIOS S
                where
                 F.FACTURA_PAGADA='N'
...
NVL((SELECT DF2.VALOR  FROM SGC_TT_DETALLE_FACTURA DF2, SGC_TP_SERVICIOS SER2  WHERE DF2.CONCEPTO=SER2.COD_SERVICIO
...
SER.COD_SERVICIO COD_SERVICIO
 ...
 FROM SGC_TT_FACTURA F, SGC_TT_DETALLE_FACTURA DF, SGC_TP_SERVICIOS SER
...
////////////////// num fac vencidas
    ...
    public function numfacven ($where,$sort,$start,$end)
    {
        $sql="SELECT *
				FROM (
					SELECT /*+ FIRST_ROWS(n) */ a.*, rownum rnum
					FROM (SELECT COUNT(distinct F.CONSEC_FACTURA) CANTIDAD, min(F.PERIODO) PERIODOMIN,max(F.PERIODO) PERIODOMAX,SUM(F.TOTAL-F.TOTAL_PAGADO+F.TOTAL_DEBITO-F.TOTAL_CREDITO) DEUDA FROM SGC_TT_FACTURA F WHERE
                    F.FACTURA_PAGADA='N' AND 
...
public function datosFacturaPdf($factura){
        $sql = "SELECT TD.DESCRIPCION_TIPO_DOC TIPODOC, TO_CHAR(f.VENCIMIENTO_NCF,'DD/MM/YYYY') VENCE_NCF,
       i.ID_TIPO_CLIENTE,I.CODIGO_INM, F.DOCUMENTO_CLIENTE DOCUMENTO,  CONCAT(N.ID_NCF,F.NCF_CONSEC)NCF, I.CATASTRO, F.NOMBRE_CLIENTE ALIAS , I.DIRECCION, U.DESC_URBANIZACION, I.ID_ZONA,
...
WHERE I.CODIGO_INM(+) = F.INMUEBLE
    ...
    public function datosFacturaAseoPdf($factura){
        $sql = "SELECT TD.DESCRIPCION_TIPO_DOC TIPODOC, TO_CHAR(f.VENCIMIENTO_NCF,'DD/MM/YYYY') VENCE_NCF,
       i.ID_TIPO_CLIENTE,I.CODIGO_INM, F.DOCUMENTO_CLIENTE DOCUMENTO,  CONCAT(N.ID_NCF,F.NCF_CONSEC)NCF, I.CATASTRO, F.NOMBRE_CLIENTE ALIAS , I.DIRECCION, U.DESC_URBANIZACION, I.ID_ZONA,
                                                                                                                TO_CHAR(F.FEC_EXPEDICION,'DD/MM/YYYY')FECEXP, F.PERIODO, I.ID_PROCESO, (SELECT G.DESC_GERENCIA FROM SGC_TT_GERENCIA_ZONA Z, SGC_TP_GERENCIAS G
WHERE G.ID_GERENCIA = Z.ID_GERENCIA AND Z.ID_ZONA = I.ID_ZONA) GERENCIA, E.DESC_MED, CA.DESC_CALIBRE, M.SERIAL, TO_CHAR(F.FEC_VCTO,'DD/MM/YYYY')FEC_VCTO,
                                                                                                                TO_CHAR(FECHA_CORTE,'DD/MM/YYYY')FECCORTE, I.ID_PROYECTO, F.MSJ_NCF, F.MSJ_PERIODO, F.MSJ_FACTURA, F.MSJ_ALERTA, F.MSJ_BURO, F.TOTAL
FROM SGC_TT_FACTURA_ASEO F, SGC_TT_INMUEBLES I, SGC_TP_NCF_USOS_ASEO N,  SGC_TP_URBANIZACIONES U, SGC_TT_MEDIDOR_INMUEBLE M, SGC_TP_MEDIDORES E, SGC_TP_CALIBRES CA,SGC_TP_TIPODOC TD
WHERE I.CODIGO_INM(+) = F.INMUEBLE
      AND CA.COD_CALIBRE(+) = M.COD_CALIBRE
      AND M.COD_INMUEBLE(+) = I.CODIGO_INM
      AND F.NCF_ID = N.ID_NCF_USO
    ...
    public function datosServiciosAseoPdf($codinm){
        $sql = "SELECT C.DESC_SERVICIO, U.DESC_USO, T.CODIGO_TARIFA, S.UNIDADES_TOT, T.CONSEC_TARIFA, U.OPERA_CORTE, C.COD_SERVICIO, S.CUPO_BASICO, A.SEC_ACTIVIDAD, A.DESC_ACTIVIDAD
		FROM SGC_TT_SERVICIOS_INMUEBLES S, SGC_TP_SERVICIOS C, SGC_TP_TARIFAS T, SGC_TP_USOS U, SGC_TP_ACTIVIDADES A, SGC_TT_INMUEBLES I
        WHERE C.COD_SERVICIO  = S.COD_SERVICIO
        AND S.CONSEC_TARIFA = T.CONSEC_TARIFA
    ...
    public function datosValorMetroAseoPdf($codinm){
        $sql = "SELECT VALOR_METRO
                FROM SGC_TT_SERVICIOS_INMUEBLES S, SGC_TP_TARIFAS T, SGC_TP_RANGOS_CATEGORIA R
                WHERE T.CONSEC_TARIFA = S.CONSEC_TARIFA
                AND R.DESCRIPCION = T.DESC_TARIFA
                AND S.COD_SERVICIO = 5
                AND S.COD_INMUEBLE = '$codinm'";
        //echo $sql;
        $resultado = oci_parse(
