...
if($factura != '') {
    ...
    $posYDatosFactura = 12; //Establece el valor a restar de Y
    ...
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Text((25 - $posXDatosFactura), (99 - $posYDatosFactura), $marmed);
    $pdf->Text((25 - $posXDatosFactura), (104 - $posYDatosFactura), $calmed);
    $pdf->Text((25 - $posXDatosFactura), (109 - $posYDatosFactura), $serial);

    $pdf->SetFont('Arial', 'B', 9);
    if ($marmed == "Sin Medidor") $pdf->Text(78, (113 - $posYDatosFactura), 'Promedio');
    else $pdf->Text(63, (113 - $posYDatosFactura), 'Diferencia Lectura');
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Text(144, (96 - $posYDatosFactura), $factura);
    $pdf->Text(142, (100 - $posYDatosFactura), $fecexp);

    $pdf->Text(189, (96 - $posYDatosFactura), $zona);
    $pdf->Text(185, (100 - $posYDatosFactura), $mes . "/" . $agno);

    $pdf->Text(158, (107 - $posYDatosFactura), $saldofavor);
    $pdf->Text(158, (111 - $posYDatosFactura), $difdeuda);
    ...
    $pdf->Ln($mueveadicion - $pxr1 - $pxr2 - $pxr3 - $pxr4 - $pxr5 - $pxcalc - $paxalcde+2);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->SetTextColor(255, 255, 255);
    ...
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Text(145, 253, $factura);
    $pdf->Text(145, 257, $mes . "/" . $agno);
    $pdf->Text(145, 261, $catastro);
    $pdf->Text(145, 265, $proceso);
    $pdf->Text(190, 253, $fecexp);
    $pdf->Text(190, 257, $codinm);
    ...
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Text(184, 274, 'RD$ ' . $totalfactura);
    $pdf->SetFont('Arial', 'B', 9);
...
//      $msjfactura=oci_result($stid, 'MSJ_FACTURA');
    ...
    if ($acueducto == 'SD' && $periodo >= 202112) {
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Text(85, 28, $fecexp, 0, 0, 'C');
        $pdf->Text(35, 33, $codinm);
        if ($tipodoc != "Cédula") {
            $pdf->Text(35, 36, $documento);
        }
        $pdf->Text(35, 42, utf8_decode(substr($alias, 0, 31)), 0, 0, 'L');
        $pdf->Text(35, 47, utf8_decode($direccion), 0, 0, 'C');
        $pdf->Text(35, 53, utf8_decode($urbaniza), 0, 0, 'C');
        $pdf->Text(140, 35, $msjncf);
        $pdf->Text(140, 40, $ncffac);
        if (trim($fecha_ncf) <> '') {
            $pdf->Text(140, 44, $fecha_ncf);
        }
        $pdf->Text(140, 49, $mes . "/" . $agno);
        $pdf->Text(140, 53, $factura);
        ...
        $pdf->Text(119, 130, $zona);
        $pdf->Text(170, 130, $fecvcto);
        ...
        while (oci_fetch($stid)) {
            ...
            if ($concepto == 'Agua' || $concepto == 'Agua de Pozo') {
                ...
                if ($rango == 0) {
                    $pdf->Ln(4);
                    $pdf->Cell(2, 5, '', 0, 0, 'L');
                    $pdf->Cell(27, 5, 'Consumo Basico', 0, 0, 'L');
                ...
                if ($rango == 1) {
                    ...
                    $pdf->Ln($pxr1);
                    $pdf->Cell(2, 5, '', 0, 0, 'L');
                    $pdf->Cell(27, 5, 'Consumo Adicional ' . $rango, 0, 0, 'L');
                    $pdf->Cell(8, 5, $unidades, 0, 0, 'R');
                    if ($unidades == 0) {
                        $valor_mts = 0;
                    } else {
            ...
            if ($concepto == 'Alcantarillado Red' || $concepto == 'Alcantarillado Pozo') {
                ...
                if ($rango >= 0) {
                    ...
                    if ($rango == 1) {
                        $distancia = $distancia - 5;
                        $pdf->Cell(27, 5, 'Consumo Adicional 1', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 2) {
                        $distancia = $distancia - 5;
                        $pdf->Cell(27, 5, 'Consumo Adicional 2', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 3) {
                        $distancia = $distancia - 5;
                        $pdf->Cell(27, 5, 'Consumo Adicional 3', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 4) {
                        $distancia = $distancia - 5;
                        $pdf->Cell(27, 5, 'Consumo Adicional 4', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    $pdf->Cell(8, 5, $unidades, 0, 0, 'R');
                    if ($unidades > 0)
        ...
        oci_free_statement($stid);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Text(72, 116, round($totalservicios));
        $pdf->Text(72, 162, round($totalotrosconceptos));
        $totalfactura = round($totalservicios + $totalotrosconceptos);
        $pdf->Text(72, 173, round($totalfactura));
        $pdf->Text(72, 225, 'RD$ ' . $totalfactura);
        $pdf->Ln($distancia);
        $pdf->Cell(98, 5, '', 0, 'L');
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Text(15, 252, utf8_decode($alias));
        $pdf->Text(127, 251, utf8_decode($alias));
        $pdf->Text(23, 257, $codinm);
        $pdf->Text(139, 256, $codinm);
        $pdf->Text(23, 261, $proceso);
        $pdf->Text(139, 260, $proceso);
        $pdf->Text(23, 267, $fecexp);
        $pdf->Text(23, 271, $factura);
        $pdf->Text(23, 275, $mes . "/" . $agno);
        $pdf->Text(23, 278, $fecvcto);
        ...
        if ($acueducto == 'SD') {
            ...
            //$pdf->Text(8, 17, 'CORPORACION DEL ACUEDUCTO Y ALCANTARILLADO DE BOCA CHICA');
            $a = new Parametro();
            $std2 = $a->getParametroByNomPar('RNC_CORAABO');
            oci_fetch($std2);
            $rcn = oci_result($std2, 'VAL_PARAMETRO');
            $pdf->SetFont('times', 'B', 8);
            $pdf->Text(8, 34, 'RNC ' . $rcn);
            $pdf->SetFont('times', 'B', 6);
        }
        $pdf->SetFont('Arial', 'B', 10);
        ...
        $pdf->SetFont('Arial', 'B', 9);
        if (trim($fecha_ncf) <> '') {
            $pdf->Text(148, 38, 'Vencimiento DEL NCF: ' . $fecha_ncf);
        }
        ...
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Text(28, 89, $marmed);
        $pdf->Text(28, 94, $calmed);
        $pdf->Text(28, 99, $serial);
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Text(141, 91, $factura);
        $pdf->Text(141, 97, $fecexp);

        $pdf->Text(189, 91, $zona);
        $pdf->Text(184, 97, $mes . "/" . $agno);


        $pdf->Text(154, 101, $saldofavor);
        $pdf->Text(154, 106, $difdeuda);
        ...
        while (oci_fetch($stid)) {
            ...
            if ($concepto == 'Agua' || $concepto == 'Agua de Pozo') {
                ...
                if ($rango == 0) {
                    $pdf->Ln(4);
                    $pdf->Cell(3, 3, '', 0, 0, 'L');
                    $pdf->Cell(41, 7, 'Consumo Basico', 0, 0, 'L');
                    $pdf->Cell(10, 5, $unidades, 0, 0, 'R');
                    if ($unidades > 0) {
                        if ($acueducto == 'SD') $valor_mts = round(($valor / $unidades), 2);
                        if ($acueducto == 'BC') $valor_mts = round(($valor / $unidades), 2);
                ...
                if ($rango == 1) {
                    ...
                    $pdf->Ln($pxr1);
                    $pdf->Cell(3, 3, '', 0, 0, 'L');
                    $pdf->Cell(38, 8, 'Consumo Adicional ' . $rango, 0, 0, 'L');
                    $pdf->Cell(10, 5, $unidades, 0, 0, 'R');
                    if ($unidades == 0) {
                        $valor_mts = 0;
                    } else {
                ...
                if ($rango == 2) {
                    ...
                    $pdf->Ln($pxr2);
                    $pdf->Cell(3, 3, '', 0, 0, 'L');
                    $pdf->Cell(41, 7, 'Consumo Adicional ' . $rango, 0, 0, 'L');
                    $pdf->Cell(10, 5, $unidades, 0, 0, 'R');
            ...
            if ($concepto == 'Alcantarillado Red' || $concepto == 'Alcantarillado Pozo') {
                ...
                if ($rango >= 0) {
                    ...
                    if ($rango == 0) {
                        $mueveadicion += 5;
                        $pdf->Cell(38, 8, 'Consumo Basico', 0, 0, 'L');
                    }
                    if ($rango == 1) {
                        $pdf->Cell(38, 8, 'Consumo Adicional 1', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 2) {
                        $pdf->Cell(38, 8, 'Consumo Adicional 2', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 3) {
                        $pdf->Cell(38, 8, 'Consumo Adicional 3', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    if ($rango == 4) {
                        $pdf->Cell(38, 8, 'Consumo Adicional 4', 0, 0, 'L');
                        $mueveadicion += 5;
                    }
                    $pdf->Cell(10, 5, $unidades, 0, 0, 'R');
                    if ($unidades > 0)
            ...
            if ($concepto != $conceptoini && ($concepto != 'Alcantarillado Red' && $concepto != 'Alcantarillado Pozo' && $concepto != 'Agua' && $concepto != 'Agua de Pozo')) {
                ...
                $pdf->Text(194, $pxotrocon, round($valor));
                $pxotrocon = $pxotrocon + 5;
                $conceptoini = $concepto;
                $totalotrosconceptos += $valor;
            }
        ...
        if ($tipoCliente != "GC" && $operaCorte != "N") {
            ...
            $pdf->Text(48.5, 188, 'AVISO');
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Text(28, 190, 'Fecha de corte de servicio ' . $fecorte);
        }
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Text(142, 240, $factura);
        $pdf->Text(142, 243, $mes . "/" . $agno);
        $pdf->Text(142, 246, $catastro);
        $pdf->Text(142, 249, $proceso);
        $pdf->Text(180, 240, $fecexp);
        $pdf->Text(183, 243, $codinm);
        ...
        $pdf->Text(65, 290, utf8_decode($alias));
        ...
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Text(180, 260, 'RD$ ' . $totalfactura);
        //$pdf->Rect(136, 273, 63, 12);
        $pdf->SetFont('Arial', 'B', 9);
        //$pdf->Text(139, 283, 'Vencimiento:');
        $pdf->Text(180, 266, $fecvcto);
        $pdf->Text(110, 267, $gerencia);
    }
    $pdf->title = "Factura";
    $pdf->Output("facturas_digital/Factura_$factura.pdf", 'I');
}
