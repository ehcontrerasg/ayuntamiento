...
$c = new PQRs();
$resultado = $c->totalfacven($inmueble);
while (oci_fetch($resultado)) {
...
$c = new PQRs();
$resultado = $c->totalfacent($inmueble);
while (oci_fetch($resultado)) {
...
$c = new PQRs();
$resultado = $c->reclamosAnteriores($inmueble);
while (oci_fetch($resultado)) {
...
$descripcion = 'Sr/Sra. ' . $nom_cli . ' solicita envio de duplicado de factura al correo ' . $mail_cli;
$cod_ent = 900;
$id_punto = 600;
$num_caja = 1;

$c = new PQRs();
$bandera = $c->IngresaPqr($fecha, $inmueble, $nom_cli, $ced_cli, $direccion, $tel_cli, $mail_cli, $medio, $tipo, $motivo, $fecha_res, $descripcion, $cod_ent, $id_punto, $num_caja, $user, $ger_inm, $area_res, $tel_cli, $mail_cli);
if ($bandera == false) {
...
if($cod_pro == 'SD'){
    ...
    $mensaje .= utf8_decode('<font size="4" face="Helvetica"><b>Estimado(a): '.$nom_cli.'</font></b><br><br>');
...
$asunto = utf8_decode('Duplicado de Factura Electrónica ').$empresa;
...
//      $msjfactura=oci_result($stid, 'MSJ_FACTURA');
    ...
    if($acueducto == 'SD' && $periodo >= 202112){
        ...
        $l=new  factura();
        $stid=$l->getDatosConsumoFactPdf($factura);
        while (oci_fetch($stid)) {
        ...
        $conceptoini = '';
        $l=new factura();
        $stid=$l->getDetFacrByFact($factura);
        $pdf->Ln(62);
        $mueveadicion = 0;
        $distancia = 68;
        $pyotro = 153;
...
$correo->enviarcorreo($mail_cli, $idpro, $asunto, $mensaje, $factura);
echo $mensaje;
...
$resolucion = 'La Factura fue enviada';
$tipo_res = 11;
$diagnostico = 1;
