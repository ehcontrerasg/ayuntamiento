...
class OtrosRec extends ConexionClass {
    public function __construct()
    {
        parent::__construct();
    }

    public function OtrosRecTotal ($where,$sort,$start,$end)
    {
       $sql="SELECT *
				FROM (
					...
					FROM (
						select * from (
						SELECT ORE.CODIGO,ORE.FECHA FECHADIG,ORE.FECHA_PAGO FECHAPAG, S.DESC_SERVICIO , ORE.IMPORTE,U.LOGIN,REV.FECHA_REV, (SELECT U2.LOGIN FROM
    ...
    ///////////////////////////////////////todos los otrosrec (para estado de cuenta)/////

    public function todosOtrosRec ($codinm)
    {
        $sql="SELECT TO_CHAR(ORE.FECHA,'DD/MM/YYYY')FECHA, TO_CHAR(ORE.FECHA,'YYYYMMDD') FECHCOMP , to_char(ORE.FECHA,'DD-MM-YYYY') FECHCOMP2  ,'pago por concepto: '||S.DESC_SERVICIO DESCRIPCION ,ORE.IMPORTE 
                FROM SGC_TT_OTROS_RECAUDOS ORE, sgc_tp_servicios s
