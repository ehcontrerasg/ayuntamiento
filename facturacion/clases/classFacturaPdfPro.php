...
$obs_lect_v    = 'Dif. Lect.';
$servicio_v    = 'Agua';
$observacion_v = 'Le informamos que adicional al cargo del mes, usted tiene un monto vencido de RD$ 32634 Evite el corte presentandose en nuestras oficinas para acordar el pago del atraso.
';
$total_v       = 1680;
$agua_v        = 1417;
$mora          = 264;
...
$fecha_v       = '14/05/2018';
$periodo_v     = 'May/2018';
$vencimiento_v = '28/05/2018';
$fecha_exp_v   = '14/05/2018';
$uso_v         = 'Comercial';
$tarifa_v      = '130';
$alias_v       = '(CABAñA DEL ESTE)';
$urbaniza_v    = 'LA CALETA';
$direccion_v   = 'AP L AMERICAS No.44';
$ciclo_v       = '26D';
$catastro_v    = '263480582000000-28';
$proceso_v     = '26533970000';
$ncf_v         = 'B0200001252';
// -----------------------------------------------------------
...
if ($factura != '') {
    ...
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Text(12, $posy, $servicio_v);
    $pdf->Text(43, $posy, $uso_v);
    $pdf->Text(68, $posy, $tarifa_v);
    $pdf->Text(87, $posy, $unidades);
    $posy = $posy + 4;
    oci_free_statement($stid);
    ...
    if ($acueducto == 'SD') {
        ...
        $pdf->Image('../../images/logo_coraabo.jpg', 74, 4, 57);
    }
    ...
    if ($acueducto == 'SD') {
        ...
        $pdf->Text(67, 28, 'CORPORACION DEL ACUEDUCTO Y ALCANTARILLADO DE BOCA CHICA');
    }

    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Text(10, 15, utf8_decode('Código Sistema'));
    $pdf->SetFont('Arial', 'B', 16);
    $pdf->Text(10, 25, $codinm_v);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Text(150, 15, 'NCF ' . $ncf_v);
    $pdf->Text(150, 20, utf8_decode('Código de Inmueble'));
    $pdf->Text(150, 25, $catastro_v);
    $pdf->Text(150, 30, utf8_decode($tipodoc) . ' ' . $documento);
    $pdf->Ln(25);
    $pdf->Rect(10, 35, 190, 15);
    $pdf->SetFont('Arial', 'B', 11);
    $pdf->Cell(90, 15, utf8_decode(substr($alias_v, 0, 31)), 0, 0, 'L');
    $pdf->Cell(50, 15, utf8_decode($direccion_v), 0, 0, 'C');
    $pdf->Cell(50, 15, utf8_decode($urbaniza_v), 0, 0, 'C');
    $pdf->Ln(18);
    ...
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(27, 5, $fecha_v, 0, 0, 'C');
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(19, 5, 'Periodo:', 0, 0, 'L');
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(19, 5, $periodo_v/*$mes . '/' . $agno*/, 0, 0, 'L');
    $pdf->Ln(8);
    ...
    if ($marmed == "Sin Medidor") {
        $pdf->Text(53, 104, 'Observacion:');
    } else {
        $pdf->Text(53, 104, 'Observacion:');
    }

    $pdf->Text(81, 104, 'Promedio');
    ...
    while (oci_fetch($stid)) {
        ...
        if ($concepto != $conceptoini && ($concepto == 'Agua' || $concepto == 'Agua de Pozo')) {
            $pdf->Cell(5, 6, '', 0, 0, 'R');
            $pdf->Cell(30, 6, $servicio_v, 0, 0, 'L');
        ...
        if ($concepto == 'Agua' || $concepto == 'Agua de Pozo') {
            ...
            if ($rango == 0) {
                ...
                $pdf->Cell(10, 5, $unidades_v, 0, 0, 'R');
                if ($unidades > 0) {
                    if ($acueducto == 'SD') {
                        $valor_mts = round(($agua_v / $unidades_v), 0);
                    }

                    if ($acueducto == 'BC') {
                        $valor_mts = round(($agua_v / $unidades_v), 4);
                    }
                ...
                if ($unidades > 0) {
                    $pdf->Cell(20, 5, round($valor_mts, 2), 0, 0, 'R');
                } else {
                    $pdf->Cell(20, 5, 0, 0, 0, 'R');
                }

                $pdf->Cell(20, 5, /*$valor*/ $agua_v, 0, 0, 'R');
            }
            ...
            if ($acueducto == 'SD') {
            $valor_mts = round(($valor / $unidades), 0);
            }

            if ($acueducto == 'BC') {
            $valor_mts = round(($valor / $unidades), 4);
            }
        ...
        $pdf->Text(110, $pxotrocon, 'Mora');
        $pdf->Text(187, $pxotrocon, $mora);
    ...
    $pdf->SetTextColor(0, 0, 0);
    if ($zona != '52A' and $zona != '52B' and $zona != '52C' and $zona != '52D' and $zona != '60A' and $zona != '60B' and $zona != '26C' and $zona != '26D' and $zona != '27D') {
        $pdf->SetFont('Arial', 'B', 9);
    ...
    $pdf->SetFont('Arial', 'B', 11);
    $totalfactura = $total_v; //round($totalservicios + $totalotrosconceptos);
    $pdf->Text(108, 183, 'TOTAL FACTURA MENSUAL');
    $pdf->Text(128, 188, '( A + B )');
    $pdf->Text(108, 198, 'Vencimiento:');
    $pdf->Text(175, 198, $vencimiento_v/*$fecvcto*/);
    ...
    //$pdf->Text(60,210,$msjncf);
    $pdf->Ln(27);
    $pdf->Cell(189, 12, $msjncf, 0, 0, 'C');

    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Ln(10);
    $pdf->MultiCell(189, 8, utf8_decode($observacion_v), 0, 'C', false);
    $pdf->Ln(-10);
    ...
    if ($acueducto == 'SD') {
        ...
        $pdf->Image('../../images/coraabo.jpg', 8, 250, 20);
    }
    ...
    $pdf->Text(137, 259, $periodo_v/*$mes . "/" . $agno*/);
    $pdf->Text(137, 263, $catastro_v);
    $pdf->Text(137, 267, $proceso_v);
    $pdf->Text(179, 255, $fecha_exp_v/*$fecexp*/);
    $pdf->Text(179, 259, $codinm_v);
    ...
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Text(40, 290, $codinm_v);
    $pdf->Text(60, 290, utf8_decode($alias_v));
    ...
    $pdf->Text(180, 286, $vencimiento_v/*$fecvcto*/);
