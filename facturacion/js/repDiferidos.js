...
function generaRep(){
    var datos=$("#repGenDifForm").serializeArray();
    //var pdf=$("#PDF").serialize();
    //var xls=$("#XLS").serialize();
    $.ajax
    ({
        url : '../reportes/reporte.diferidos.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        success : function(json) {
            $('#repGenDifSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repGenDifSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
