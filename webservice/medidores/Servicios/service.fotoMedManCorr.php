...
$target_path = "./../../fotos_sgc/foto_med_man_corr/".$carpetafecha."/";
...
if(!file_exists($target_path)){
 ...
 if(move_uploaded_file($_FILES['fotoUp']['tmp_name'], $target_path1)) {
 	...
 	$resultado=$foto->existefotoManCorr($consecutivofoto, $orden);
 	oci_fetch($resultado);
 	...
 	else
 	{
 	    ...
 	    $foto->CambiarEstadoFotoManCorr($codsistema);
 	    $foto->EliminarFotoManCorr($codsistema);
        ...
        $foto->seturl($urlfoto);
        $foto->setorden($orden);
        $fotos=$foto->insertarfotoManCorr();
 		$bandera=array(array('bandera'=>'1'));
 		echo json_encode($bandera);
 	}
 }else{
