...
if(!file_exists($target_path)){
 ...
 if(move_uploaded_file($_FILES['fotoUp']['tmp_name'], $target_path1)) {
 	...
 	$resultado=$foto->existefotoManPre($consecutivofoto, $orden,$fechaformato." ".$fecha);
 	oci_fetch($resultado);
 	...
 	{
        $foto->CambiarEstadoFotoManPre($codsistema);
 	    $foto->setconsecutivo($consecutivofoto);
        $foto->setcodsistema($codsistema);
        $foto->setfecha($fechaformato." ".$fecha);
        $foto->setnombre($nombrefoto);
        $foto->seturl($urlfoto);
        $foto->setorden($orden);
        $fotos=$foto->insertarfotoManPre();
 		$bandera=array(array('bandera'=>'1'));
 		echo json_encode($bandera);
 	}
 }else{
