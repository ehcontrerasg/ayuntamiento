...
class Medidor extends ConexionClass
{
    ...
    public function IngresaMedManPre($usr,$orden,$medio,$fecIns,$med,$cal,$ser,$lect,$emp,$obins,$long,$lat,$inm,$obsImpe){


          $sql="BEGIN SGC_P_INGMANTMED_MOV('$usr',$orden,'$medio',$lect,'$fecIns','$med','$cal','$ser','$emp','$obins','','$long','$lat','$inm','$obsImpe',:PMSGRESULT,:PCODRESULT);COMMIT;END;";
        $nombrelog=date('Y-m-d');
    ...
    public function IngresaActInsMed($act,$ord){


        $sql="BEGIN SGC_P_INGACTINSMED_MOV('$act',$ord,:PMSGRESULT,:PCODRESULT);COMMIT;END;";
        $resultado= oci_parse($this->_db,$sql);
        oci_bind_by_name($resultado,":PMSGRESULT",$this->mesrror,500);
        oci_bind_by_name($resultado,":PCODRESULT",$this->coderror);
        $bandera=oci_execute($resultado);
