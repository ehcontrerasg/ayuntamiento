...
function selecionarAcu(valor)
{
    ...
    // recorremos todos los valores del select
    for(var i=1;i<$('#selAcue > option').length;i++)

    {
        var select=document.getElementById("selAcue");
        for(var i=1;i<select.length;i++)
        {
...
function llenarSpinerAcue()
{
    ...
    ({
        ...
        success : function(json) {
            $('#selAcue').empty();
            $('#selAcue').append(new Option('', '', true, true));
            ...
            for(var x=0;x<json.length;x++)
            {
                ...
                $('#selAcue').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
