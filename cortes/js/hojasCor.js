...
function inicio(){
    ...
    iframePdf=document.getElementById("ifHojaCor");
    ...
    butProcesar=document.getElementById("butOrdCor");
    butProcesar.addEventListener("click",procHojaCor);

    radbMetodo=document.getElementsByName("rbModo");
    formHojaCor=document.getElementById("formHojasCor");
    llenarSelProy();
}

function procHojaCor(){
    ...
    iframePdf.src="../datos/datos.hojaCor.php?tipo="+tipo+"&proy="+pro+"&sec="+sec+"&zon="+zon+"&oper="+oper;
}
...
function  cambiaSelZona(){
    ...
    spinner = new Spinner(opts).spin(formHojaCor);
    formHojaCor.appendChild(spinner.el);
    llenarSelOperario();
}
...
function llenarSelProy(){
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            formHojaCor.removeChild(spinner.el);
            habilitarBotones();
        }
    ...
    xmlhttp.open("POST", "../datos/datos.hojaCor.php", true);   // async
