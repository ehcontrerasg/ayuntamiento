...
function generaRep(){
    ...
    $.ajax
    ({
        url : '../reportes/reportes.rendCor.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
...
function llenarSpinerCon()
{
    ...
    ({
        ...
        data : { tip : 'selCon' },
        success : function(json) {
            $('#selContAntMed').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#selContAntMed').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
