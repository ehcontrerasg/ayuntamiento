...
function initMap() {
    ...
    parametros.push({name: 'periodo', value: periodo});
    parametros.push({name: 'usr', value: usuario});
    parametros.push({name: 'tip', value: 'corrCor'});
...
function cargSectores()
{
    ...
    ({
        ...
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selSec' , pro:$('#rendCorSelPro').val() },
        success : function(json) {
            $('#rendCorSelSec').empty();
            $('#rendCorSelSec').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rendCorSelSec').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function flexyRendCor(){

    var parametros =
        $("#rendCorForm").serializeArray();
        parametros.push({name: 'tip', value: 'flexy'});
        parametros.push({name: 'fecIni', value: $("#rendCorInpFecIni").val()});
        parametros.push({name: 'fecFin', value: $("#rendCorInpFecFin").val()});
        parametros.push({name: 'tip', value: 'flexy'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendCor").flexigrid
    (
        {
            url: '../datos/datos.rendCort.php',
            dataType: 'json',
            ...
            colModel :
                [
                    ...
                    {display: 'Periodo', name: 'ruta', width: 36, sortable: true, align: 'center'},
                    ...
                    {display: 'Predios<br>Leidos', name: 'leidos', width: 34, sortable: true, align: 'center'},
            ...
            height: 245,
            rp: 1000,
            sortname: "TO_CHAR(MIN(RC.FECHA_EJE),'DD/MM/YYYY HH24:MI:SS')",
            sortorder: "DESC",
            params: parametros
        }
    );
    $("#flexRendCor").flexOptions({url: '../datos/datos.rendCort.php'});
    $("#flexRendCor").flexOptions({params: parametros});
    $("#flexRendCor").flexReload();
}



function flexRendCorDet(){
    ...
    parametros.push({name: 'periodo', value: periodo});
    parametros.push({name: 'usr', value: usuario});
    parametros.push({name: 'contratista', value: $("#rendCorSelContr").val()});
    parametros.push({name: 'tip', value: 'flexyDetRend'});


    $('.flexme1').flexigrid();
    $('.flexme2').flexigrid({height:'auto',striped:false});
    $("#flexRendCorDet").flexigrid
    (
        {
            url: '../datos/datos.rendCort.php',
            dataType: 'json',
            ...
            colModel :
                [
                    ...
                    {display: 'Lectura',  width: 31, sortable: false, align: 'center'},
                    {display: 'Tipo corte',  width: 39, sortable: false, align: 'center'},
                    {display: 'Impo Corte',  width: 52, sortable: false, align: 'center'},
                    {display: 'Observacion',  width: 100, sortable: false, align: 'center'},
                    {display: 'Fecha Corte',  width: 89, sortable: false, align: 'center'},
                    {display: 'Fotos',  width: 54, sortable: false, align: 'center'}
                ],
            usepager: true,
            title: 'Detalle Cortes Operario ruta' ,
            useRp: false,
            page: 1,
            showTableToggleBtn: false,
            width: 1000,
    ...
    $("#flexRendCorDet").flexOptions({url: '../datos/datos.rendCort.php'});
    $("#flexRendCorDet").flexOptions({title: 'Detalle Cortes Operario ruta:'+ruta});
    $("#flexRendCorDet").flexOptions({params: parametros});
    $("#flexRendCorDet").flexReload();
}



function asigevenRend() {
    tabflexPag = document.getElementById("flexRendCor");
