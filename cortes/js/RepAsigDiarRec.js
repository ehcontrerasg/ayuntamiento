...
function generaImp(){
    ...
    $.ajax
    ({
        url : '../reportes/reporte.repAsigDiarRec.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
        success : function(urlPdf) {
            ...
            if (urlPdf.substr(0,11)=="../../temp/"){
                ...
                $("#PdfRepAsigRec").prop('data',urlPdf) ;
            }else{
                swal
                (
                    {
...
function llenarSpinerPro()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.repAsigDiariaRec.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selPro' },
        success : function(json) {
...
function llenarSpinerContr()
{
    ...
    ({
        ...
        data : { tip : 'selCon' },
        success : function(json) {
            $('#rutPdcSelContr').empty();
            $('#rutPdcSelContr').append(new Option('', '', true, true));


            for(var x=0;x<json.length;x++)
            {

                $('#rutPdcSelContr').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
...
function llenarSpinerGer()
{
    ...
    ({
        ...
        success : function(json) {
            $('#rutPdcSelGer').empty();
            $('#rutPdcSelGer').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#rutPdcSelGer').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
