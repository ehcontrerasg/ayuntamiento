...
$(document).ready(function(){
    ...
    function generaImp(){
        var datos=$("#genRepMedCorForm").serializeArray();
        $.ajax
        ({
            url : '../reportes/reporte.repCortConLect.php',
            type : 'POST',
            dataType : 'text',
            data : datos ,
            success : function(urlPdf) {
                ...
                if (urlPdf.substr(0,11)=="../../temp/"){
                    ...
                    if($("#genRepMedCorSelFor").val()=='xls'){
                        window.location.href = urlPdf;
                    }else if ($("#genRepMedCorSelFor").val()=='pdf'){
                        $("#PdfRepGenMedCorInpProIni").prop('data',urlPdf) ;
                    }
    ...
    $("#genRepMedCorInpProIni").blur(
        function(){
           complementaInpProFin()
        }
    )
    $("#genRepMedCorInpProFin").blur(
        function(){
...
{
    var faltante =11-$("#genRepMedCorInpProIni").val().length;
    $("#genRepMedCorInpProFin").val(($("#genRepMedCorInpProIni").val()+faltanteFunc('9',faltante)).substr(0,11));
    $("#genRepMedCorInpProIni").val( ($("#genRepMedCorInpProIni").val()+faltanteFunc('0',faltante)).substr(0,11));
}

function complementaInpProFin2()
{
    var faltante =11-$("#genRepMedCorInpProFin").val().length;
    $("#genRepMedCorInpProFin").val(($("#genRepMedCorInpProFin").val()+faltanteFunc('9',faltante)).substr(0,11));
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#genRepMedCorSelPro').empty();
            $('#genRepMedCorSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#genRepMedCorSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
