...
function llenarSpinerGer()
{
    ...
    $.ajax
    ({
        url : '../datos/datos.repAsigDiariaCort.php',
        type : 'POST',
        dataType : 'json',
        data : { tip : 'selGer' },
        success : function(json) {
            $('#repDiarCorSelGer').empty();
            $('#repDiarCorSelGer').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repDiarCorSelGer').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
...
function generaImp(){
    var datos=$("#genRepAsigDiarCor").serializeArray();
    $.ajax
    ({
        url : '../reportes/reporte.repAsigDiarCor.php',
        type : 'POST',
        dataType : 'text',
        data : datos ,
        success : function(urlPdf) {
            ...
            if (urlPdf.substr(0,11)=="../../temp/"){
                ...
                $("#PdfRepAsigCor").prop('data',urlPdf) ;
            }else{
                swal
                (
                    {
...
function llenarSpinerPro()
{
    ...
    ({
        ...
        data : { tip : 'selPro' },
        success : function(json) {
            $('#repDiarCorSelPro').empty();
            $('#repDiarCorSelPro').append(new Option('', '', true, true));
            for(var x=0;x<json.length;x++)
            {
                $('#repDiarCorSelPro').append(new Option(json[x]["DESCRIPCION"], json[x]["CODIGO"], false, false));
            }
        },
        error : function(xhr, status) {
