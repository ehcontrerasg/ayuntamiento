...
$c = new Pqr();
$resultado = $c->getDatTotPqrByPqr ($cod_pqr);
while (oci_fetch($resultado)) {
	...
	$pdf->Ln(4);
	$pdf->SetFont('Arial','',8);
	$descrip1 = substr($descripcion,0,109);
	$descrip2 = substr($descripcion,109,strlen($descripcion));
	...
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(90,5,utf8_decode($cliente),0,0,'L');
	$pdf->SetFont('Arial','B',8);
	...
	//$pdf->Cell(95,5,"_________________________________________",0,0,'C');
    $pdf->Cell(95,5,utf8_decode($nomusuariorec),0,0,'C');
    $pdf->Cell(95,5,utf8_decode($cliente),0,0,'C');
	$pdf->Ln(3);
