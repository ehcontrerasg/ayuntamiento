...
<form action="acea_google.php"  name="formulario" method="post" >
 ...
 $sql="SELECT * FROM (
 SELECT LONGITUD, LATITUD,CODIGO_INM
 FROM SGC_TT_INSPECCIONES_CORTES IC
 WHERE IC.CODIGO_INM IN ($cod_inms)  AND IC.FECHA_ASIG between to_date('$fecini','YYYY-MM-DD')  and to_date('$fecfin','YYYY-MM-DD')
 and IC.USR_ASIG='$operario'
  AND LONGITUD <> 0
 ...
 while (oci_fetch($stid)) {
 	$xi = oci_result($stid, 'LONGITUD');
   	...
   	$id_sistema_ini = oci_result($stid, 'CODIGO_INM');
 ...
 $sql="SELECT * FROM (
 SELECT LONGITUD, LATITUD,CODIGO_INM ID_INMUEBLE
 FROM SGC_TT_INSPECCIONES_CORTES IC
 WHERE IC.CODIGO_INM IN ($cod_inms)  AND IC.FECHA_ASIG between to_date('$fecini','YYYY-MM-DD')  and to_date('$fecfin','YYYY-MM-DD')
 and IC.USR_ASIG='$operario'
 AND LONGITUD <> 0
 ORDER BY FECHA_EJE desc) 
 WHERE ROWNUM = 1";
 //ECHO $sql;
 $stid = oci_parse($link, $sql);
 oci_execute($stid, OCI_DEFAULT);
 ...
 $sql="SELECT LONGITUD, LATITUD,CODIGO_INM
 FROM SGC_TT_INSPECCIONES_CORTES
 WHERE CODIGO_INM IN ($cod_inms)  AND FECHA_ASIG between to_date('$fecini','YYYY-MM-DD')  and to_date('$fecfin','YYYY-MM-DD')
 and USR_ASIG='$operario'
  AND LONGITUD <> 0
 ORDER BY FECHA_EJE ASC";
 //ECHO $sql;
 $stid = oci_parse($link, $sql);
 ...
 while (oci_fetch($stid)) {
	$x = oci_result($stid, 'LONGITUD');
   	...
   	$cod_sistema = oci_result($stid, 'CODIGO_INM');
