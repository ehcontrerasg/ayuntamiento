...
//x=xgeo;
    ...
    <form action="acea_google.php"  name="formulario" method="post" >
        ...
        $l=new Corte();
        $registros=$l->getPriPuntCoorByRutFecUsu($cod_ruta, $fechaini, $fechafin, $operario);
        while (oci_fetch($registros)) {
            $xi = oci_result($registros, 'LONGITUD');
            $yi = oci_result($registros, 'LATITUD');
            $id_sistema_ini = oci_result($registros, 'ID_INMUEBLE');
        ...
        $l=new Corte();
        $registros=$l->getUltPuntCoorByRutFecUsu($cod_ruta, $fechaini, $fechafin, $operario);
        while (oci_fetch($registros)) {
            $xi = oci_result($registros, 'LONGITUD');
            $yi = oci_result($registros, 'LATITUD');
        ...
        $l=new Corte();
        $registros=$l->getResPuntCoorByRutFecUsu($cod_ruta, $fechaini, $fechafin, $operario,$id_sistema_ini,$id_sistema_fin);
        $i=0;
        while (oci_fetch($registros)) {
            $i=$i+1;
        ...
        $array_coordenadas = "";
        $l=new Corte();
        $registros=$l->getTotPuntCoorByRutFecUsu($cod_ruta, $fechaini, $fechafin, $operario);
        while (oci_fetch($registros)) {
            $x = oci_result($registros, 'LONGITUD');
            $y = oci_result($registros, 'LATITUD');
            $array_y =  "$y";
