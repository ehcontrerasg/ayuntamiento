...
if($tip=="sec")
{
    ...
    $q=new Sector();
    $datos = $q->getSecHojCorByPro($proy);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
        $sectores[$i]=$row;
        $i++;
...
if($tip=="ope")
{
    ...
    $q=new Usuario();
    $datos = $q->getOperCorByProSecZon($proy,$sec,$zon);
    $i=0;
    while ($row = oci_fetch_array($datos, OCI_ASSOC+OCI_RETURN_NULLS)) {
        $operarios[$i]=$row;
        $i++;
...
if($tip=="M")
{
    ...
    $rutAnt="";
	$s=new Corte();
    $stid=$s->getCantInmueblesACor($proy,$sec,$zon,$oper);
	 while (oci_fetch($stid))
    {
        $cantInm=oci_result($stid, 'NUMERO');
	}
    $s=new Corte();
    $stid=$s->getInmACorByProSecZonUsu($proy,$sec,$zon,$oper);
    $numhoja=0;
    $tamCuadro=0;
    while (oci_fetch($stid))
    {
...
if($tip=="P")
{
    ...
    $s3=new Corte();
    $dat=$s3->getCantInmueblesACor($proy,$sec,$zon,$oper);
    while (oci_fetch($dat))
    {
